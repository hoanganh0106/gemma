...
**Đính chính correctness K2 All3 (rà soát source):** Không coi K2 All3 là frontier hợp lệ cho đầu vào tổng quát. Các nhận xét thứ hạng All3 K2 bên dưới chỉ mô tả timing trên fixture hiện tại, không phải khuyến nghị dùng implementation này.

- Snapshot `all3_job53647_81909_38271_210595/source/src/device/qkv/xnorm8.rs:16-35` không đọc `_rms_weight`, chỉ load x, cast f32, `BitAnd(-0.5)` rồi gather một term f8. `proj8.rs` nhận `m![H]`, không cộng term dư q2. Comment nói RMSNorm/hai term không khớp thân hàm.
- `scripts/generate_references.py:250-262` chủ động dựng `x = bf16(signs / input_rms_weight)`, nên activation chuẩn gần một bội vô hướng chung của signs. Dòng 279-290 xác nhận Q, K và cả V đều qua head-RMSNorm sau projection; V không có norm weight. Scale chung dương gần như bị triệt tiêu, nhưng không chính xác tuyệt đối do epsilon và làm tròn.
- `BitAnd(-0.5)` là mask bit dấu/exponent, không phải hàm sign tổng quát: chỉ trên miền exponent phù hợp mới cho `sign(x) * 0.5`. Bỏ weight và thông tin magnitude làm sai ngữ nghĩa tổng quát. Với x cố định, đổi weight theo từng phần tử có thể đổi hướng activation chuẩn, trong khi All3 vẫn cho cùng kết quả vì không đọc weight; head-RMSNorm không khôi phục được hướng bị mất.
- Vì vậy, PASS 10/10 trên fixture này không chứng minh correctness trên dữ liệu thật. Min K2 `76,677` cycles thuộc phép tính đã bỏ RMSNorm và q2, không chứng minh schedule tốt hơn. Chưa có phép đo cô lập để quy toàn bộ chênh lệch cycles cho từng phần bị bỏ. Một term f8 tự nó không đủ kết luận sai; bỏ weight/magnitude là lỗi quyết định ở đây.
- Nếu fixture đổi hoặc code được kiểm tra theo ngữ nghĩa yêu cầu, K2 All3 có nguy cơ không đạt. Giữ đường RMSNorm có weight của merge làm cơ sở tối ưu; không đề xuất toàn bộ snapshot All3 làm frontier leaderboard. K2 baseline/candidate giống source, nên chênh lệch timing giữa chúng cũng không chứng minh cải tiến implementation K2.

Lần rà soát này xác minh source, chưa chạy thêm test đầu vào độc lập hoặc Arena. Các raw samples bên dưới được giữ nguyên làm dữ liệu lịch sử.
