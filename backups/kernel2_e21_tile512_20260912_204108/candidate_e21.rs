...
fn output_partial(
    ...
    let x = x.tile::<m![Qs], 512, m![Qs = 512 # 4096]>(offset);
    let weight_f8: DmTensor<f8e4m3, Chip, Cluster, HiddenRows, m![H % 120, Qs = 512]> = weight
        .view()
        .tile::<m![Qs], 512, m![H, Qs = 512 # 4096]>(offset)
        .to_dm(&mut ctx.tdma);
    let x_trf: TrfTensor<bf16, Chip, Cluster, HiddenRows, m![1], m![Qs = 512]> = ctx
        .sub
        .begin(x)
        .fetch::<m![1], m![Qs = 512]>()
        .collect::<m![Qs = 512 / 16], m![Qs = 512 % 16]>()
        .to_trf();
    ctx.main
        .begin(weight_f8.view())
        .fetch::<m![H % 120, Qs = 512 / 32], m![Qs = 512 % 32]>()
        .fetch_table_lookup::<bf16>()
        .collect::<m![H % 120, Qs = 512 / 16], m![Qs = 512 % 16]>()
        .contract_outer::<m![H % 120, Qs = 512 / 32], m![Qs = 512 % 32], _, _, _>(&x_trf)
        .contract_packet::<m![1]>()
