...
pub(crate) fn project_normalize_add(
    ...
    let residual_dm: TailDm<bf16> = residual_hbm.to_dm(&mut ctx.tdma);
    let residual: TailVrf = ctx
        .sub
    ...
    let z_gathered: DmTensor<bf16, Chip, OutputClusters, m![1 # 256], m![H % 1920]> =
        z.to_dm(&mut ctx.tdma);
    z_gathered.view().to_hbm_view(&mut ctx.tdma, hop.view_mut());

    // These two are what the model puts between the store and the reload: they cover the sync.
    let scale_dm: TailDm<bf16> = weight_scale.to_dm(&mut ctx.tdma);
    let scale: TailVrf = ctx
        .sub
        .begin(scale_dm.view())
        .fetch::<m![1], m![H % 480]>()
        .fetch_cast::<f32>()
    ...
    let rms_dm: TailDm<bf16> = rms_weight.to_dm(&mut ctx.tdma);
    let rms_weight: TailVrf = ctx
        .sub
        .begin(rms_dm.view())
        .fetch::<m![1], m![H % 480]>()
        .fetch_cast::<f32>()
    ...
    let inv_rms: DmTensor<f32, Chip, Vc, m![1 # 32, Vr], m![1 # 8]> = ctx
        .main
        .begin(partial.view())
        .fetch::<m![1], m![1 # 8]>()
        .collect::<m![1], m![1 # 8]>()
