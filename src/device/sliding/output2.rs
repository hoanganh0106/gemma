...
use super::output::{Lq, Ob, Oc, Og, Ok, Or, Ow};
...
pub(crate) fn contract_tile_a(
    ...
    y: &mut DmTensor<f32, Chip, OutputClusters, Rows, m![H % 120, 1 # 8]>,
    offset: usize,
) {
    // fewer-ops A1: the two level lanes leave the contraction one after the other in TIME
    // (Sequential), the vector engine adds them while they are f32, then the sixteen column
    // partials meet in the inter-slice reduce. One value per 32-byte packet is committed as it
    // is (no transpose: a transpose after intra + inter reduce ICEs the perf model).
    ctx.main
        .begin(weight.view())
        ...
        .vector_final()
        .commit_trim::<m![1 # 8]>()
        .commit_view(y.view_mut().tile::<m![H % 120], 96, m![H % 120 = 96 #{!} 120, 1 # 8]>(offset));
}
...
pub(crate) fn contract_tile_b(
    ...
    ctx.main
        ...
        .vector_final()
        .commit_trim::<m![1 # 8]>()
        .commit_view(y.view_mut().tile::<m![H % 120], 24, m![H % 120 = 24 #{!} 120, 1 # 8]>(offset));
}
...
pub(crate) fn project_quantised(
...
) -> DmTensor<f32, Chip, OutputClusters, Rows, m![H % 120, 1 # 8]> {
    ...
    let mut y: DmTensor<f32, Chip, OutputClusters, Rows, m![H % 120, 1 # 8]> = DmTensor::new();
