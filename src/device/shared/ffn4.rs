...
) -> TrfTensor<f8e4m3, Chip, UpGateClusters, Gathered, m![1], m![T2, Ut, Xp]> {
...
/// The same for the gate, with its global scale riding the pass. The scalar is loaded into one
/// slice of every group (a cheap strided load) and handed to the other three by the Switch; a
...
pub(crate) fn feedforward(
    ...
    let up = fold4(ctx, &up);
    let gate = fold4(ctx, &gate);

    let up = regroup(ctx, &up);
    let gate = regroup_scaled(ctx, &gate, gate_global_scale);
    ...
    let mut down_z: DmTensor<f32, Chip, UpGateClusters, DownSlices, m![H / 2 % 15, Dt, Xp / 16]> = DmTensor::new();
    let mut partial: DmTensor<f32, Chip, UpGateClusters, DownSlices, m![H / 2 % 15, Xp / 16]> = DmTensor::new();
    down_block_sums!(ctx, down_weight_packed, x_trf, down_z, 0);
    down_scale_tile!(ctx, down_z, down_scale, partial, 0, 4);
    down_block_sums!(ctx, down_weight_packed, x_trf, down_z, 5);
    down_scale_tile!(ctx, down_z, down_scale, partial, 4, 4);
    down_block_sums!(ctx, down_weight_packed, x_trf, down_z, 10);
    down_scale_tile!(ctx, down_z, down_scale, partial, 8, 4);
    down_scale_tile!(ctx, down_z, down_scale, partial, 12, 3);
