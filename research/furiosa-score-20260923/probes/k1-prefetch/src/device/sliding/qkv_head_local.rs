...
pub(crate) type NativeQueryWeight = DmTensor<f8e4m3, Chip, QkvDualQueryCluster, NativeSlices, m![Qs % 8, H]>;
pub(crate) type NativeKvWeight = DmTensor<f8e4m3, Chip, QkvDualQueryCluster, NativeSlices, m![Ps % 4, H]>;

pub(crate) fn prefetch_projection_weights(
    device: &mut Device,
    q_weight: &HbmTensor<f8e4m3, Chip, m![Qs, H]>,
    k_weight: &HbmTensor<f8e4m3, Chip, m![Ps, H]>,
    v_weight: &HbmTensor<f8e4m3, Chip, m![Ps, H]>,
) -> (NativeQueryWeight, NativeKvWeight, NativeKvWeight) {
    // Preserve the original DMA mappings and reshape wire order. Move the three
    // independent loads ahead of projection consumers to test DMA scheduling.
    // Q/K/V occupy 30 KiB + 15 KiB + 15 KiB on each of 512 active slices.
    let q: DmTensor<f8e4m3, Chip, QkvDualQueryCluster, m![Qs / 8 % 256], m![Qs % 8, H]> =
        q_weight.to_dm(&mut device.tdma);
    let k: DmTensor<f8e4m3, Chip, KvCluster, KvRows, m![Ps % 4, H]> =
        k_weight.to_dm(&mut device.tdma);
    let v: DmTensor<f8e4m3, Chip, KvCluster, KvRows, m![Ps % 4, H]> =
        v_weight.to_dm(&mut device.tdma);
    unsafe { (q.reshape(), k.reshape(), v.reshape()) }
}
...
pub(crate) fn project_query(
    ...
    let heads: DmTensor<bf16, Chip, HeadCluster, m![Ns % 4, Gs, Dummy256 % 32], m![Ds]> = device.main
        .begin(rows.view())
        .fetch::<m![1], m![Ds % 8]>()
    ...
    scale_head(device, &heads, &weight_scale)
}
...
pub(crate) fn project_key_value(
    ...
    x_trf: &NativeTrf,
    x_scale: &NativeScale,
    k_weight: &NativeKvWeight,
    v_weight: &NativeKvWeight,
    k_weight_scale: &HbmTensor<bf16, Chip, m![Ps]>,
    v_weight_scale: &HbmTensor<bf16, Chip, m![Ps]>,
) -> (KvHeadTensor, KvHeadTensor) {
    let k = project_one_kv_matrix(device, x_trf, x_scale, k_weight, k_weight_scale);
    let v = project_one_kv_matrix(device, x_trf, x_scale, v_weight, v_weight_scale);
    (k, v)
}
...
fn rope_one<S: M>(
    ...
    let sin_product: DmTensor<f32, Chip, HeadCluster, S, m![Ds]> = device.main
        .begin(rotated.view())
        .fetch::<m![Ds / 16], m![Ds % 16]>()
        .fetch_cast::<f32>()
        .collect::<m![Ds / 8], m![Ds % 8]>()
...
pub(crate) fn apply_rope(
    ...
    let cos_row: HbmTensor<bf16, Chip, m![Ds]> = cos_row.to_hbm(&mut device.tdma);
    let sin_row: HbmTensor<bf16, Chip, m![Ds]> = sin_row.to_hbm(&mut device.tdma);
    let q_cos: QueryHeadTensor = cos_row.to_dm(&mut device.tdma);
...
pub(crate) fn normalize_native_input(
    ...
    let mean: DmTensor<f32, Chip, QkvDualQueryCluster, Copies, m![1 # 8]> = device.main
        ...
        .commit_trim::<m![1 # 8]>()
        .commit();
    let rms: DmTensor<f32, Chip, QkvDualQueryCluster, Copies, m![1 # 8]> = device.main
        .begin(mean.view())
        .fetch::<m![1], m![1 # 8]>()
        .collect::<m![1], m![1 # 8]>()
        .vector_init()
    ...
    let rms: DmTensor<f32, Chip, QkvDualQueryCluster, Shards, m![1 # 8]> =
        unsafe { rms.reshape() };
    let rms_vrf: VrfTensor<f32, Chip, QkvDualQueryCluster, Shards, m![1 # 8]> = device.sub
        .begin(rms.view())
        .fetch::<m![1], m![1 # 8]>()
        .collect::<m![1], m![1 # 8]>()
        .to_vrf();
    ...
    let scale_vrf: NativeScale = device.sub
        .begin(scale.view())
        .fetch::<m![1], m![1 # 8]>()
        .collect::<m![1], m![1 # 8]>()
        .to_vrf();
