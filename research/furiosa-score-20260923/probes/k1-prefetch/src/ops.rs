...
pub fn sliding_project_qkv(
    ...
    let (q_weight_dm, k_weight_dm, v_weight_dm) = sliding::qkv_head_local::prefetch_projection_weights(
        device, q_weight, k_weight, v_weight,
    );
    let (x, x_scale) = sliding::qkv_head_local::normalize_native_input(device, x, input_rms_weight);

    let q = sliding::qkv_head_local::project_query(device, &x, &x_scale, &q_weight_dm, q_weight_scale);
    let (k, v) = sliding::qkv_head_local::project_key_value(
        device, &x, &x_scale, &k_weight_dm, &v_weight_dm, k_weight_scale, v_weight_scale,
    );

    let q = sliding::qkv_head_local::normalize_weighted(device, &q, q_rms_weight);
    let k = sliding::qkv_head_local::normalize_weighted(device, &k, k_rms_weight);
    let v = sliding::qkv_head_local::normalize_value(device, &v);
    let (q, k) = sliding::qkv_head_local::apply_rope(device, &q, &k, rope_offset, cos, sin);
