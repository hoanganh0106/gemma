...
axes![Lq = 2, Vr = 8, HopCluster = 2, HopHalf = 2, HopH = 1920];
...
pub(crate) fn project_normalize_add(
    ...
    // SDK 0.8.1 feasibility probe: assemble [local, peer] row halves using SRAM DMA.
    // Cluster 0 receives rows [0..1920, 1920..3840]; cluster 1 receives the reverse
    // order and is discarded below. No padded slice is asserted to contain real data.
    let z: DmTensor<bf16, Chip, m![HopCluster], m![HopH / 120, 1 # 16], m![HopHalf = 1, HopH % 120]> =
        unsafe { z.reshape() };
    let mut z_all: DmTensor<bf16, Chip, m![HopCluster], m![1 # 256], m![HopHalf, HopH]> = DmTensor::new();
    z.view().to_dm_view(
        &mut device.tdma,
        z_all.view_mut().tile::<m![HopHalf], 1, m![HopHalf = 1 #{!} 2, HopH]>(0),
    );
    z.view().cluster_swap().to_dm_view(
        &mut device.tdma,
        z_all.view_mut().tile::<m![HopHalf], 1, m![HopHalf = 1 #{!} 2, HopH]>(1),
    );
    // Only cluster 0, slice 0 is live in this view. Its 3840 contiguous values are
    // in global H row order. Converting a real second cluster to padding drops it.
    let z_all: DmTensor<bf16, Chip, Vc, m![1 # 256], m![H]> = unsafe { z_all.reshape() };
    ...
    let z_tail: TailDm<bf16> = z_all.to_dm(&mut device.tdma);
    ...
    let inv_rms: DmTensor<f32, Chip, Vc, m![1 # 32, Vr], m![1 # 8]> = device
        .main
        .begin(partial.view())
        .fetch::<m![1], m![1 # 8]>()
        .collect::<m![1], m![1 # 8]>()
    ...
    let inv_rms_vrf: VrfTensor<f32, Chip, Vc, Tail, m![1 # 8]> = device
        .sub
        .begin(inv_rms.view())
        .fetch::<m![1], m![1 # 8]>()
        .collect::<m![1], m![1 # 8]>()
    ...
    let out: TailDm<bf16> = device
        ...
        .commit_trim::<m![H % 8]>()
        .commit();
    let out: DmTensor<bf16, Chip, m![1 # 2], Tail, m![H % 480]> = unsafe { out.reshape() };
