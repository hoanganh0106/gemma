...
pub(crate) type NativeTrf = TrfTensor<f8e4m3, Chip, QkvDualQueryCluster, NativeSlices, m![1], m![H]>;
...
pub(crate) fn project_query(
    ...
    let weight_dm: DmTensor<f8e4m3, Chip, HeadCluster, QueryStripe, m![Ds / 8 % 8, H]> =
        weight_view.to_dm(&mut device.tdma);
    let x_rows: &TrfTensor<f8e4m3, Chip, HeadCluster, QueryStripe, m![1], m![H]> =
        unsafe { std::mem::transmute(x_trf) };

    let sums: DmTensor<f32, Chip, HeadCluster, QueryStripe, m![Ds / 8 % 8, 1 # 8]> = device.main
    ...
    let sorted: DmTensor<bf16, Chip, HeadCluster,
        m![Ns % 4, Gs, Ds / 64, Ds / 8 % 8], m![Ds % 8]> = device.main
        .begin(sums.view())
        .fetch::<m![Ds / 8 % 8], m![1 # 8]>()
        .switch::<m![Ns % 4, Gs, Ds / 64, Ds / 8 % 8], m![Ds % 8]>(
            SwitchConfig::InterTranspose { slice1: 8, slice0: 1, time0: 1 }
        )
    ...
    let scale_dm: QueryHeadTensor = scale_view.to_dm(&mut device.tdma);
...
pub(crate) fn copy_query(device: &mut Device, x: &QueryHeadTensor) -> QueryHeadTensor {
    device.main
        .begin(x.view())
        .fetch::<m![Ds / 16], m![Ds % 16]>()
...
fn project_one_kv_matrix_scaled(
    ...
    let x_trf: &TrfTensor<f8e4m3, Chip, HeadCluster, KvStripe, m![1], m![H]> =
        unsafe { std::mem::transmute(x_trf) };
    let sums: DmTensor<f32, Chip, HeadCluster, KvStripe, m![Ds / 8 % 4, 1 # 8]> = device.main
        .begin(weight.view())
        ...
        .commit_trim::<m![1 # 8]>()
        .commit();
    let sorted: DmTensor<bf16, Chip, HeadCluster, m![Ns % 4, Ds / 32, Ds / 8 % 4, Ds / 4 % 2], m![Ds % 4]> = device.main
        .begin(sums.view())
        .fetch::<m![Ds / 8 % 4], m![1 # 8]>()
        .switch::<m![Ns % 4, Ds / 32, Ds / 8 % 4, Ds / 4 % 2], m![Ds % 4]>(
            SwitchConfig::InterTranspose { slice1: 4, slice0: 2, time0: 1 }
        )
    ...
    let scale_view: HbmTensorView<'_, bf16, Chip, m![Ns, Ds]> =
        unsafe { weight_scale.view().reshape() };
    let scale_dm: KvHeadTensor = scale_view.to_dm(&mut device.tdma);
...
fn apply_projection_scale_kv(
    ...
    let weight_scale: DmTensor<bf16, Chip, HeadCluster, KvHeads, m![Ds]> = weight_scale.to_dm(&mut device.tdma);
    scale_head(device, x, &weight_scale)
}
...
pub(crate) fn project_key_value(
    ...
    let k = project_one_kv_matrix_raw(device, x_trf, k_weight);
    let v_scaled = project_one_kv_matrix_scaled(device, x_trf, v_weight, v_weight_scale);
    let v = normalize_value(device, &v_scaled);
    (k, v)
}
...
fn root_mean_square<S: M>(
    ...
    let mean_square: DmTensor<f32, Chip, HeadCluster, S, m![1 # 8]> = device.main
        ...
        .commit_trim::<m![1 # 8]>()
        .commit();
    let rms: DmTensor<f32, Chip, HeadCluster, S, m![1 # 8]> = device.main
...
fn normalize_weighted_project_scaled<S: M>(
    ...
    let proj_vrf: VrfTensor<f32, Chip, HeadCluster, S, m![Ds]> = device.sub
        .begin(proj_scale.view())
        .fetch::<m![Ds / 16], m![Ds % 16]>()
        .fetch_cast::<f32>()
        .collect::<m![Ds / 8], m![Ds % 8]>()
        .to_vrf();
    let norm_dm: DmTensor<bf16, Chip, HeadCluster, S, m![Ds]> = norm_weight.to_dm(&mut device.tdma);
    let norm_vrf: VrfTensor<f32, Chip, HeadCluster, S, m![Ds]> = device.sub
        .begin(norm_dm.view())
        .fetch::<m![Ds / 16], m![Ds % 16]>()
        .fetch_cast::<f32>()
        .collect::<m![Ds / 8], m![Ds % 8]>()
    ...
    let rms: DmTensor<f32, Chip, HeadCluster, S, m![1 # 8]> = device.main
        ...
        .commit_trim::<m![1 # 8]>()
        .commit();
    let rms_vrf: VrfTensor<f32, Chip, HeadCluster, S, m![1 # 8]> = device.sub
        .begin(rms.view())
        .fetch::<m![1], m![1 # 8]>()
        .collect::<m![1], m![1 # 8]>()
        .to_vrf();
...
pub(crate) fn normalize_query_project_scaled(
    ...
    let dm: DmTensor<bf16, Chip, HeadCluster, QueryHeads, m![Ds]> = v.to_dm(&mut device.tdma);
    normalize_weighted_project_scaled(device, x, &dm, norm_weight)
}
...
pub(crate) fn normalize_key_project_scaled(
    ...
    let dm: DmTensor<bf16, Chip, HeadCluster, KvHeads, m![Ds]> = v.to_dm(&mut device.tdma);
    normalize_weighted_project_scaled(device, x, &dm, norm_weight)
}
...
fn normalize_value_project_scaled(
    ...
    let weight_scale: HbmTensorView<'_, bf16, Chip, m![Ns, Ds]> = unsafe { weight_scale.view().reshape() };
    let weight_dm: DmTensor<bf16, Chip, HeadCluster, KvHeads, m![Ds]> = weight_scale.to_dm(&mut device.tdma);
    let weight_vrf: VrfTensor<f32, Chip, HeadCluster, KvHeads, m![Ds]> = device.sub
        .begin(weight_dm.view())
        .fetch::<m![Ds / 16], m![Ds % 16]>()
        .fetch_cast::<f32>()
        .collect::<m![Ds / 8], m![Ds % 8]>()
    ...
    let mean_square: DmTensor<f32, Chip, HeadCluster, KvHeads, m![1 # 8]> = device.main
        .begin(x.view())
        .fetch::<m![Ds / 16], m![Ds % 16]>()
        .fetch_cast::<f32>()
        .collect::<m![Ds / 8], m![Ds % 8]>()
        ...
        .commit_trim::<m![1 # 8]>()
        .commit();
    let rms: DmTensor<f32, Chip, HeadCluster, KvHeads, m![1 # 8]> = device.main
        .begin(mean_square.view())
        .fetch::<m![1], m![1 # 8]>()
        .collect::<m![1], m![1 # 8]>()
        .vector_init()
        ...
        .commit_trim::<m![1 # 8]>()
        .commit();
    let rms_vrf: VrfTensor<f32, Chip, HeadCluster, KvHeads, m![1 # 8]> = device.sub
        .begin(rms.view())
        .fetch::<m![1], m![1 # 8]>()
        .collect::<m![1], m![1 # 8]>()
        .to_vrf();
...
fn rope_one<S: M>(
    ...
    let cos_vrf: VrfTensor<f32, Chip, HeadCluster, S, m![Ds]> = device.sub
        ...
        .collect::<m![Ds / 8], m![Ds % 8]>()
        .to_vrf();
    let s0_vrf: VrfTensor<f32, Chip, HeadCluster, S, Half> = device.sub
        .begin(sin.view().tile::<m![Ds], 128, m![Ds = 128 # 256]>(0))
        .fetch::<m![1], m![Ds = 128]>()
        .fetch_cast::<f32>()
        .collect::<m![Ds = 128 / 8], m![Ds = 128 % 8]>()
        .to_vrf();
    let s1_vrf: VrfTensor<f32, Chip, HeadCluster, S, Half> = device.sub
    ...
    device.main
        ...
        .commit_view(sin_product.view_mut().tile::<m![Ds], 128, m![Ds = 128 #{!} 256]>(0));
    device.main
        ...
        .vector_final()
        .commit_trim::<m![Ds = 128 % 8]>()
        .commit_view(sin_product.view_mut().tile::<m![Ds], 128, m![Ds = 128 #{!} 256]>(128));

    let sin_product_vrf: VrfTensor<f32, Chip, HeadCluster, S, m![Ds]> = device.sub
...
fn rope_one_pair_fixed<S: M>(
    ...
    let cos_vrf: VrfTensor<f32, Chip, HeadCluster, S, Half> = device.sub
        .begin(pair.view().tile::<m![Ds], 128, m![Ds = 128 # 256]>(0))
        .fetch::<m![1], Half>()
        .fetch_cast::<f32>()
        .collect::<m![Ds = 128 / 8], m![Ds = 128 % 8]>()
        .to_vrf();
    let sin_vrf: VrfTensor<f32, Chip, HeadCluster, S, Half> = device.sub
    ...
    let sin0_vrf: VrfTensor<f32, Chip, HeadCluster, S, Half> = device.sub
        .begin(sin_product.view().tile::<m![Ds], 128, m![Ds = 128 # 256]>(0))
        .fetch::<m![1], Half>()
        .collect::<m![Ds = 128 / 8], m![Ds = 128 % 8]>()
        .to_vrf();
    let sin1_vrf: VrfTensor<f32, Chip, HeadCluster, S, Half> = device.sub
        .begin(sin_product.view().tile::<m![Ds], 128, m![Ds = 128 # 256]>(128))
        .fetch::<m![1], Half>()
        .collect::<m![Ds = 128 / 8], m![Ds = 128 % 8]>()
        .to_vrf();
    ...
    device.main
        ...
        .cast::<bf16, m![Ds = 128 % 8 # 16]>()
        .commit_trim::<m![Ds = 128 % 8]>()
        .commit_view(out.view_mut().tile::<m![Ds], 128, m![Ds = 128 #{!} 256]>(128));
    out
}
...
pub(crate) fn apply_rope(
    ...
    type Half = m![Ds = 128];
    type HalfTable = m![65536, Ds = 128];

    // RoPE tables are [a,a] for cos and [-b,+b] for the supplied sin table.
    // Reinterpret [E,256] as [2E,128]. The original byte offset pos*512
    // selects row 2*pos automatically, i.e. the first half, with no index math.
    let cos_half_table: &HbmTensor<bf16, Chip, HalfTable> = unsafe { std::mem::transmute(cos) };
    let sin_half_table: &HbmTensor<bf16, Chip, HalfTable> = unsafe { std::mem::transmute(sin) };

    let cos_half: DmTensor<bf16, Chip, Cluster, Slice, Half> =
        cos_half_table.dma_gather_scaled(rope_offset);
    let sin_half: DmTensor<bf16, Chip, Cluster, Slice, Half> =
        sin_half_table.dma_gather_scaled(rope_offset);

    // Pack cos_half and sin_half into one 256-element row before crossing HBM.
    // This replaces two HBM stores + two replicated loads with one of each.
    let mut rope_pair: DmTensor<bf16, Chip, Cluster, Slice, m![Ds]> = DmTensor::new();
    device.main
        .begin(cos_half.view())
        .fetch::<m![1], Half>()
        .collect::<m![Ds = 128 / 16], m![Ds = 128 % 16]>()
        .commit_trim::<m![Ds = 128 % 16]>()
        .commit_view(rope_pair.view_mut().tile::<m![Ds], 128, m![Ds = 128 #{!} 256]>(0));
    device.sub
        .begin(sin_half.view())
        .fetch::<m![1], Half>()
        .collect::<m![Ds = 128 / 16], m![Ds = 128 % 16]>()
        .commit_trim::<m![Ds = 128 % 16]>()
        .commit_view(rope_pair.view_mut().tile::<m![Ds], 128, m![Ds = 128 #{!} 256]>(128));
    let rope_pair_hbm: HbmTensor<bf16, Chip, m![Ds]> = rope_pair.to_hbm(&mut device.tdma);
    let q_pair: QueryHeadTensor = rope_pair_hbm.to_dm(&mut device.tdma);

    let q = rope_one_pair_fixed(device, q, &q_pair);
    let k_pair: &KvHeadTensor = unsafe { std::mem::transmute(&q_pair) };
    let k = rope_one_pair_fixed(device, k, k_pair);
    (q, k)
}
...
pub(crate) fn normalize_native_input(
    ...
    _weight: &HbmTensor<bf16, Chip, m![H]>,
) -> NativeTrf {
    ...
    // Experimental fast path: quantize x directly and omit the input-weight DMA/multiply.
    let quantized: DmTensor<f8e4m3, Chip, QkvDualQueryCluster, Shards, m![H % 120]> = device.main
        .begin(x.view())
    ...
    // Broadcast the one FP8 component to all native slices.
    let shared: DmTensor<f8e4m3, Chip, QkvDualQueryCluster, Copies, m![H / 8 % 15, H / 120, H % 8]> = device.main
        .begin(quantized.view())
        .fetch::<m![H / 8 % 15], m![H % 8]>()
        .switch::<Copies, m![H / 8 % 15, H / 120]>(SwitchConfig::Broadcast1 { slice1: 32, slice0: 1 })
        .collect::<m![H / 8 % 15, H / 120], m![H % 8 # 32]>()
        .commit_trim::<m![H % 8]>()
        .commit();
    let shared: DmTensor<f8e4m3, Chip, QkvDualQueryCluster, NativeSlices, m![H / 8 % 15, H / 120, H % 8]> =
        unsafe { shared.reshape() };
    let dense: DmTensor<f8e4m3, Chip, QkvDualQueryCluster, NativeSlices, m![H]> = device.main
        .begin(shared.view())
        .fetch::<m![H / 8], m![H % 8]>()
