...
fn project_one_kv_matrix(
    device: &mut Device,
    x_trf: &NativeTrf,
    x_scale: &VrfTensor<f32, Chip, HeadCluster, m![Ns % 4, Ds / 32, Ds % 4, Ds / 4 % 2], m![1 # 8]>,
    ...
    let x_trf: &TrfTensor<f8e4m3, Chip, HeadCluster, KvStripe, m![Dummy2], m![H]> =
    ...
    let sums: DmTensor<f32, Chip, HeadCluster, KvStripe, m![Ds / 8 % 4, 1 # 8]> = device.main
        ...
        .contract_lane::<m![Ds / 8 % 4, Dummy2], m![1 # 8]>(LaneMode::Sequential)
        .vector_init()
        .vector_intra_slice_tag(TagMode::Zero)
        .vector_narrow_trim::<m![1 # 4]>()
        .vector_fp_binary(FpBinaryOp::MulF(FpMulAlu::Mul0), x_scale)
        .vector_intra_slice_reduce::<Dummy2, m![Ds / 8 % 4], m![1 # 4]>(IntraSliceReduceOpF32::Add)
        .vector_widen_pad::<m![1 # 8]>()
        .vector_final()
        .commit_trim::<m![1 # 8]>()
        .commit();
...
pub(crate) fn project_key_value(
    ...
    // Consuming the handle changes only its logical labels; all physical units
