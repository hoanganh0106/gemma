...
pub(crate) fn project_query(
    ...
    let x_trf = query_operand(device, x);

    let weight: HbmTensorView<'_, f8e4m3, Chip, m![Ns, Gs, Ds, H]> = unsafe { weight.view().reshape() };
    let weight_f8: DmTensor<f8e4m3, Chip, QueryClusters, QueryRowSlices, m![Ds / 8 % 8, H]> = weight.to_dm(&mut device.tdma);

    let contraction = contract_query(device, &weight_f8, &x_trf);
...
fn contract_kv_matrix(
    ...
    let contraction = contract_key_value(device, store.view().tile::<m![Kv], 1, m![Kv = 1 # 2, Ds / 8 % 4, H]>(tile), x_trf);
...
fn project_value_matrix(
    ...
    let contraction = contract_kv_matrix(device, x_trf, weight, 1, store);
    device.main
        .begin(contraction.view())
        .fetch::<m![1], m![Ds % 4]>()
        .fetch_cast::<f32>()
...
fn project_key_matrix(
    ...
    let contraction = contract_kv_matrix(device, x_trf, weight, 0, store);
...
pub(crate) fn project_key_value(
    ...
    let x_trf = key_value_operand(device, x);

    let mut k_store: KvPad = DmTensor::new();
    let mut v_store: KvPad = DmTensor::new();
    project_key_matrix(device, &x_trf, k_weight, k_weight_scale, pool, &mut k_store);
    project_value_matrix(device, &x_trf, v_weight, v_weight_scale, &mut v_store)
}
...
fn key_value_operand(
    ...
    device.sub.begin(x).fetch::<m![Term], m![H]>().collect::<m![Term, H / 32], m![H % 32]>().to_trf()
}
...
fn contract_query(
    ...
    let sums: DmTensor<f32, Chip, QueryClusters, QueryRowSlices, m![Ds / 8 % 8, 1 # 8]> = device
        .main
    ...
    let sorted: DmTensor<bf16, Chip, QueryClusters, m![Ns % 4, Gs, Ds / 64, Ds / 8 % 8], m![Ds % 8]> = device
        .main
        .begin(sums.view())
...
fn contract_key_value(
    ...
    let sums: DmTensor<f32, Chip, KeyValueClusters, KeyValueRowSlices, m![Ds / 8 % 4, 1 # 8]> = device
        .main
        .begin(weight_f8)
    ...
    let sorted: DmTensor<bf16, Chip, KeyValueClusters, m![Ns % 4, Ds / 32, Ds / 8 % 4, Ds / 4 % 2], m![Ds % 4]> = device
        .main
        .begin(sums.view())
