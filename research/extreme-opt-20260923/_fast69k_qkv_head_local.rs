...
use crate::axes::{Qs, Dummy2, Dummy8, Dummy256};
use crate::device::layout::Replicated;
use crate::{Chip, EPS};
use super::projection::QkvDualQueryCluster;
...
pub(crate) fn project_query(
    ...
    let sums: DmTensor<f32, Chip, HeadCluster, QueryStripe, m![Ds / 8 % 8, 1 # 8]> = ctx.main
    ...
    let sorted: DmTensor<bf16, Chip, HeadCluster,
        m![Ns % 4, Gs, Ds / 64, Ds / 8 % 8], m![Ds % 8]> = ctx.main
        .begin(sums.view())
    ...
    let scale_vrf: VrfTensor<f32, Chip, HeadCluster, QueryHeads, m![Ds]> = ctx.sub
        .begin(scale_dm.view())
...
pub(crate) fn copy_query(ctx: &mut Context, x: &QueryHeadTensor) -> QueryHeadTensor {
    ctx.main
        .begin(x.view())
        .fetch::<m![Ds / 16], m![Ds % 16]>()
...
fn project_one_kv_matrix_scaled(
    ...
    let x_trf: &TrfTensor<f8e4m3, Chip, HeadCluster, KvStripe, m![1], m![H]> =
        unsafe { std::mem::transmute(x_trf) };
    let sums: DmTensor<f32, Chip, HeadCluster, KvStripe, m![Ds / 8 % 4, 1 # 8]> = ctx.main
        .begin(weight.view())
        ...
        .commit_trim::<m![1 # 8]>()
        .commit();
    let sorted: DmTensor<bf16, Chip, HeadCluster, m![Ns % 4, Ds / 32, Ds / 8 % 4, Ds / 4 % 2], m![Ds % 4]> = ctx.main
        .begin(sums.view())
    ...
    let scale_vrf: VrfTensor<f32, Chip, HeadCluster, KvHeads, m![Ds]> = ctx.sub
        .begin(scale_dm.view())
        .fetch::<m![Ds / 16], m![Ds % 16]>()
        .fetch_cast::<f32>()
        .collect::<m![Ds / 8], m![Ds % 8]>()
...
fn project_one_kv_matrix_raw(
    ...
    let heads: DmTensor<bf16, Chip, HeadCluster, m![Ns % 4, Dummy256 % 64], m![Ds]> = ctx.main
        .begin(rows.view())
        .fetch::<m![1], m![Ds % 4]>()
...
fn apply_projection_scale_kv(
    ...
    let weight_scale: DmTensor<bf16, Chip, HeadCluster, KvHeads, m![Ds]> = weight_scale.to_dm(&mut ctx.tdma);
...
fn root_mean_square<S: M>(
    ...
    let mean_square: DmTensor<f32, Chip, HeadCluster, S, m![1 # 8]> = ctx.main
        .begin(x.view())
        .fetch::<m![Ds / 16], m![Ds % 16]>()
        .fetch_cast::<f32>()
        .collect::<m![Ds / 8], m![Ds % 8]>()
...
fn normalize_weighted_project_scaled<S: M>(
    ...
    let proj_vrf: VrfTensor<f32, Chip, HeadCluster, S, m![Ds]> = ctx.sub
    ...
    let norm_vrf: VrfTensor<f32, Chip, HeadCluster, S, m![Ds]> = ctx.sub
        .begin(norm_dm.view())
        .fetch::<m![Ds / 16], m![Ds % 16]>()
        .fetch_cast::<f32>()
        .collect::<m![Ds / 8], m![Ds % 8]>()
    ...
    let rms: DmTensor<f32, Chip, HeadCluster, S, m![1 # 8]> = ctx.main
        ...
        .commit_trim::<m![1 # 8]>()
        .commit();
    let rms_vrf: VrfTensor<f32, Chip, HeadCluster, S, m![1 # 8]> = ctx.sub
        .begin(rms.view())
        .fetch::<m![1], m![1 # 8]>()
        .collect::<m![1], m![1 # 8]>()
        .to_vrf();
...
fn normalize_value_project_scaled(
    ...
    let weight_scale: HbmTensorView<'_, bf16, Chip, m![Ns, Ds]> = unsafe { weight_scale.view().reshape() };
    let weight_dm: DmTensor<bf16, Chip, HeadCluster, KvHeads, m![Ds]> = weight_scale.to_dm(&mut ctx.tdma);
    let weight_vrf: VrfTensor<f32, Chip, HeadCluster, KvHeads, m![Ds]> = ctx.sub
        .begin(weight_dm.view())
        .fetch::<m![Ds / 16], m![Ds % 16]>()
        .fetch_cast::<f32>()
        .collect::<m![Ds / 8], m![Ds % 8]>()
    ...
    let mean_square: DmTensor<f32, Chip, HeadCluster, KvHeads, m![1 # 8]> = ctx.main
        .begin(x.view())
        .fetch::<m![Ds / 16], m![Ds % 16]>()
        .fetch_cast::<f32>()
        .collect::<m![Ds / 8], m![Ds % 8]>()
        ...
        .commit_trim::<m![1 # 8]>()
        .commit();
    let rms: DmTensor<f32, Chip, HeadCluster, KvHeads, m![1 # 8]> = ctx.main
        .begin(mean_square.view())
        .fetch::<m![1], m![1 # 8]>()
        .collect::<m![1], m![1 # 8]>()
        .vector_init()
        ...
        .commit_trim::<m![1 # 8]>()
        .commit();
    let rms_vrf: VrfTensor<f32, Chip, HeadCluster, KvHeads, m![1 # 8]> = ctx.sub
        .begin(rms.view())
        .fetch::<m![1], m![1 # 8]>()
        .collect::<m![1], m![1 # 8]>()
        .to_vrf();
...
fn rope_one<S: M>(
    ...
    type Half = m![Ds = 128];
    let cos_vrf: VrfTensor<f32, Chip, HeadCluster, S, m![Ds]> = ctx.sub
        .begin(cos.view())
        .fetch::<m![Ds / 16], m![Ds % 16]>()
        .fetch_cast::<f32>()
        .collect::<m![Ds / 8], m![Ds % 8]>()
        .to_vrf();
    let s0_vrf: VrfTensor<f32, Chip, HeadCluster, S, Half> = ctx.sub
        .begin(sin.view().tile::<m![Ds], 128, m![Ds = 128 # 256]>(0))
        .fetch::<m![1], m![Ds = 128]>()
        .fetch_cast::<f32>()
        .collect::<m![Ds = 128 / 8], m![Ds = 128 % 8]>()
        .to_vrf();
    let s1_vrf: VrfTensor<f32, Chip, HeadCluster, S, Half> = ctx.sub
    ...
    let sin_product_vrf: VrfTensor<f32, Chip, HeadCluster, S, m![Ds]> = ctx.sub
...
fn rope_one_pair_fixed<S: M>(
    ...
    let cos_vrf: VrfTensor<f32, Chip, HeadCluster, S, Half> = ctx.sub
        .begin(pair.view().tile::<m![Ds], 128, m![Ds = 128 # 256]>(0))
        .fetch::<m![1], Half>()
        .fetch_cast::<f32>()
        .collect::<m![Ds = 128 / 8], m![Ds = 128 % 8]>()
        .to_vrf();
    let sin_vrf: VrfTensor<f32, Chip, HeadCluster, S, Half> = ctx.sub
    ...
    let sin0_vrf: VrfTensor<f32, Chip, HeadCluster, S, Half> = ctx.sub
        .begin(sin_product.view().tile::<m![Ds], 128, m![Ds = 128 # 256]>(0))
        .fetch::<m![1], Half>()
        .collect::<m![Ds = 128 / 8], m![Ds = 128 % 8]>()
        .to_vrf();
    let sin1_vrf: VrfTensor<f32, Chip, HeadCluster, S, Half> = ctx.sub
...
pub(crate) fn apply_rope(
    ...
    let rope_pair_hbm: HbmTensor<bf16, Chip, m![Ds]> = rope_pair.to_hbm(&mut ctx.tdma);
    let q_pair: QueryHeadTensor = rope_pair_hbm.to_dm(&mut ctx.tdma);

    let q = rope_one_pair_fixed(ctx, q, &q_pair);
    let k_pair: &KvHeadTensor = unsafe { std::mem::transmute(&q_pair) };
    let k = rope_one_pair_fixed(ctx, k, k_pair);
    (q, k)
}
...
pub(crate) fn normalize_native_input(
    ...
    // The later per-head RMSNorms remove global scale, but not elementwise variation.
    let quantized: DmTensor<f8e4m3, Chip, QkvDualQueryCluster, Shards, m![H % 120]> = ctx.main
        .begin(x.view())
    ...
    let shared: DmTensor<f8e4m3, Chip, QkvDualQueryCluster, Copies, m![H / 8 % 15, H / 120, H % 8]> = ctx.main
    ...
    let dense: DmTensor<f8e4m3, Chip, QkvDualQueryCluster, NativeSlices, m![H]> = ctx.main
        .begin(shared.view())
        .fetch::<m![H / 8], m![H % 8]>()
