...
fn project_one_kv_matrix(
    ...
    x_trf: &NativeTrf,
    x_scale: &NativeScale,
    weight: &DmTensor<f8e4m3, Chip, QkvDualQueryCluster, NativeSlices, m![Ps % 4, H]>,
...
pub(crate) fn project_key_value(
    ...
    // Queue both matrix transfers before K compute so the V transfer can overlap
    // K contraction and channel scaling. Preserve the established DMA mappings.
    let k_loaded: DmTensor<f8e4m3, Chip, KvCluster, KvRows, m![Ps % 4, H]> =
        k_weight.to_dm(&mut device.tdma);
    let v_loaded: DmTensor<f8e4m3, Chip, KvCluster, KvRows, m![Ps % 4, H]> =
        v_weight.to_dm(&mut device.tdma);
    let k_loaded: DmTensor<f8e4m3, Chip, QkvDualQueryCluster, NativeSlices, m![Ps % 4, H]> =
        unsafe { k_loaded.reshape() };
    let v_loaded: DmTensor<f8e4m3, Chip, QkvDualQueryCluster, NativeSlices, m![Ps % 4, H]> =
        unsafe { v_loaded.reshape() };
    let k = project_one_kv_matrix(device, x_trf, x_scale, &k_loaded, k_weight_scale);
    let v = project_one_kv_matrix(device, x_trf, x_scale, &v_loaded, v_weight_scale);
    (k, v)
}
