...
- P187 swapped the P166 output packet32 order from `fetch(...).collect(...)` to `collect(...).fetch(...)`.
- Clean SDK 0.8.1 build rejected the chain at Rust type checking: `collect` exists but trait bounds are unsatisfied after the order swap.
