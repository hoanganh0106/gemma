...
## P092 RMS producer placement probe

P092 changes only the `rms` producer context from Main to Sub while preserving P078's lane and gather layout. The SDK 0.8.1 release build completed, but graph initialization failed before schedule generation with `gather src residue has no live target axis`. The context change invalidates the downstream layout/dataflow invariant and is rejected before Arena.
