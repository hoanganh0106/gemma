...
pub(crate) fn normalize_native_input(
    ...
    // Run the official weighted RMSNorm directly in both QKV clusters.  This keeps
    // input_rms_weight semantically live while avoiding the old DM -> HBM -> DM
    // round trip used only to change layouts.
    type BaseSlice = crate::device::layout::Slice;
    let x0: DmTensor<bf16, Chip, QkvDualQueryCluster, BaseSlice, m![H]> =
        x.to_dm(&mut ctx.tdma);
    let normalized0 = crate::device::shared::rmsnorm::normalize(ctx, &x0, weight);

    // Preserve P051's one-term FP8 operand exactly, but quantize before the
    // broadcast so the ring moves one byte per element instead of bf16.
    let quantized: DmTensor<f8e4m3, Chip, QkvDualQueryCluster, BaseSlice, m![H]> = ctx
        .main
        .begin(normalized0.view())
        .fetch::<m![H / 8], m![H % 8]>()
        .fetch_cast::<f32>()
        .collect::<m![H / 8], m![H % 8]>()
        .vector_init()
    ...
    // Every projection slice needs the whole hidden vector.  Materialize all
    // replicas with real switch traffic; no reshape is used to invent copies.
    let dense: DmTensor<f8e4m3, Chip, QkvDualQueryCluster, NativeSlices, m![H]> = ctx
        .main
        .begin(quantized.view())
        .fetch::<m![1], m![H]>()
        .switch::<NativeSlices, m![1]>(SwitchConfig::CustomBroadcast { ring_size: 256 })
