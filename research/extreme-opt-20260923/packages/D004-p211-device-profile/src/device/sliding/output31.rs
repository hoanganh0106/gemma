...
pub(crate) type WeightHalf =
    DmTensor<f8e4m3, Chip, OutputClusters, SlidingOutputColumns, m![H % 120 = 60, Qs % 256]>;

/// Contract one 60-row half of every row slice.  Two independent weight allocations let TDMA
/// prefetch the second half while MAIN consumes the first half.
pub(crate) fn contract_half(
    device: &mut Device,
    weight: &WeightHalf,
    x_trf: &XTrf,
    z: &mut Z,
    ...
    device.main
        ...
        .contract_outer::<m![H % 120 = 60, Qs / 32 % 8], m![Qs % 32], _, _, _>(x_trf)
        .contract_packet::<m![1]>()
        .contract_time::<m![H % 120 = 60]>()
        .contract_lane::<m![H % 120 = 60, Lq], m![1 # 8]>(LaneMode::Sequential)
        .vector_init()
        .vector_intra_slice_tag(TagMode::Zero)
        .vector_narrow_trim::<m![1 # 4]>()
        .vector_intra_slice_reduce::<Lq, m![H % 120 = 60], m![1 # 4]>(IntraSliceReduceOpF32::Add)
        .vector_widen_pad::<m![1 # 8]>()
        .vector_inter_slice_reduce::<Rows, m![H % 120 = 60]>(InterSliceReduceOpF32::Add)
        .vector_final()
        .cast::<bf16, m![1 # 16]>()
        .transpose::<m![H % 120 = 60 / 4], m![H % 120 = 60 % 4 # 16]>()
        .commit_trim::<m![H % 120 = 60 % 4]>()
        .commit_view(
            z.view_mut()
                .tile::<m![H % 120], 60, m![H % 120 = 60 #{!} 120]>(row_offset),
        );
}
...
pub(crate) fn project_normalize_add(
    ...
    // Split each 120-row slice into two independent 60-row HBM->DM transfers.  The second
    // transfer is independent of the first contraction, allowing TDMA and MAIN to overlap.
    let weight_lo: WeightHalf = weight
        .view()
        .tile::<m![H % 120], 60, m![H / 120, H % 120 = 60 # 120, Qs]>(0)
        .to_dm(&mut device.tdma);
    let weight_hi: WeightHalf = weight
        .view()
    ...
    // 16 * (weight @ x), bf16.  Each half commits directly into its rows of z.
    let mut z: Z = DmTensor::new();
    contract_half(device, &weight_lo, &x_trf, &mut z, 0);
    contract_half(device, &weight_hi, &x_trf, &mut z, 60);
