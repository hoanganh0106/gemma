...
fn quantize_gather(
    ...
    let gathered: DmTensor<bf16, Chip, UpGateClusters, DownSlices, m![T2 = 1, Xd]> = device
        ...
        .collect::<m![T2 = 1, Xd / 24 % 5, Xd / 120], m![Xd % 24 # 16]>()
        .commit_trim::<m![Xd % 24]>()
        .commit();
...
macro_rules! down_tile {
    ($device:ident, $w:ident, $x_trf:ident, $scale:ident, $out:ident, $start:literal, $len:literal) => {{
        ...
        let packed: DmTensor<f4e2m1, Chip, UpGateClusters, DownSlices, m![H / 2 % 15 = $len, Dt, Xp]> =
            unsafe { packed.reshape() };
        let weight_f8: DmTensor<f8e4m3, Chip, UpGateClusters, DownSlices, m![H / 2 % 15 = $len, Dt, Xp]> = $device
            .main
            .begin(packed.view())
            ...
            .commit_trim::<m![Xp % 32]>()
            .commit();
        let weight_bf16: DmTensor<bf16, Chip, UpGateClusters, DownSlices, m![H / 2 % 15 = $len, Dt, Xp]> = $device
            .main
