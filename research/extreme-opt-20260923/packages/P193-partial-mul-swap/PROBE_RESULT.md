...
## Regenerated static schedule

- Command: `cargo furiosa-opt compile ops::sliding_attention_output --exact --dump-schedule schedule-p166-current.json`.
- Source current at compile time: P166 packet32 coupled mapping.
- Result: compile succeeded; current makespan is `21341` cycles, with the final HBM store ending at `20741` and the core epilogue ending at `21341`.
- Artifact: `schedule-p166-current.json`; the older `schedule-k2-p126.json` remains historical and must not be used as P166 current-source evidence.

## Arena control after ring timeout probes — job 96044

- P166 binary SHA256: `5965D4748E2428EF04A59223599BC7A67A6BC8AF7728DDC9CCDCA29A4321422F`.
- All 15 correctness checks passed.
- K1 median `80130`; K2 median `37697` (`[37763, 37697, 37519]`); K3 median `216374`.
- P166 completed normally in the same hardware window in which P178/P179 timed out during K2, confirming the timeout is caused by their ring/tail mechanisms rather than a transient Arena outage.
- Result artifact: Arena job `96044`.
- P193 swapped the two partial RMS square multipliers: `scale` uses `Mul1`, `Stash` uses `Mul0`.
- Binary SHA256: `C97B7CCCD4DD8708B3C477B4B7CB074A811EE1ED8AE30BB68C28D05F9E6F61A1`.
- Arena job `96827` passed all 15/15 correctness checks.
- Hardware medians: K1 `80535`, K2 `37515` (`[37161, 37515, 38660]`), K3 `216367`.
- P193 is `182` cycles faster than fresh P166 control job `96044` K2 `37697`; keep as provisional frontier pending paired repeat. Result artifact: `arena-96827.result.txt`.
- Repeat Arena job `96835` passed all 15/15 correctness checks.
- Repeat hardware medians: K1 `80445`, K2 `37546` (`[37067, 37546, 38904]`), K3 `217009`.
- P193 remains faster than P166 control `96044` by `151` cycles on K2; fresh P166 adjacent control `96843` was submitted for paired confirmation. Result artifact: `arena-96835.result.txt`.
- Adjacent P166 control job `96843` also measured K2 `37546` (`[36977, 37546, 38261]`) with 15/15 correctness, exactly tying P193 repeat. The earlier `182`-cycle lead was hardware noise; P193 is not promoted.
