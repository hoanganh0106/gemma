...
pub(crate) fn normalize_query(
    ...
    let mean_square: DmTensor<f32, Chip, Cluster, Slice, m![Gs, 1 # 8]> = device
        .main
        .begin(x.view())
    ...
    let rms: DmTensor<f32, Chip, Cluster, Slice, m![Gs, 1 # 8]> = device
        .main
    ...
    let rms_vrf: VrfTensor<f32, Chip, Cluster, Slice, m![Gs, 1 # 8]> = device
        .sub
        .begin(rms.view())
        .fetch::<m![Gs], m![1 # 8]>()
        .collect::<m![Gs], m![1 # 8]>()
...
pub(crate) fn normalize_key(
    ...
    let rms_vrf = root_mean_square_tile(device, super::pool::tile(pool, super::pool::K_HEAD));
...
fn root_mean_square_tile(device: &mut Device, x: super::pool::Tile<'_>) -> VrfTensor<f32, Chip, Cluster, Slice, m![1 # 8]> {
    let mean_square: DmTensor<f32, Chip, Cluster, Slice, m![1 # 8]> = device
        .main
        .begin(x)
        .fetch::<m![Pool = 1, Ds / 16], m![Ds % 16]>()
