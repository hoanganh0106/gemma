...
macro_rules! block_sums_from_dm {
    ($device:ident, $packed_dm:ident, $x_trf:ident) => {{
        let packed: DmTensor<f4e2m1, Chip, UpGateClusters, Gathered, m![L % 30, Ut, Xp]> =
            unsafe { $packed_dm.reshape() };
...
pub(crate) fn feedforward(
    ...
    let up_weight_dm: DmTensor<f4e2m1, Chip, UpGateClusters, RowSlices, m![L % 30, H]> =
        up_weight_packed.to_dm(&mut device.tdma);
    let up_z = block_sums_from_dm!(device, up_weight_dm, x_trf);
    // Stage the independent gate matrix while the Main context can consume the up projection.
    let gate_weight_dm: DmTensor<f4e2m1, Chip, UpGateClusters, RowSlices, m![L % 30, H]> =
        gate_weight_packed.to_dm(&mut device.tdma);
    let up_s = scales_bf16(device, up_weight_scale);
    let up = apply_scales(device, &up_z, &up_s);
    let gate_z = block_sums_from_dm!(device, gate_weight_dm, x_trf);
    let gate_s = scales_bf16(device, gate_weight_scale);    let gate = apply_scales(device, &gate_z, &gate_s);
