...
- P189 changed the output contract factorization from `Qs / 32 % 8` to `Qs / 16 % 16` while retaining packet32.
- Clean SDK 0.8.1 build reached VISA but failed lowering `TensorUnit`: `Fail to cover valid range` because the fetch source uses `Qs_1=16` while the weight buffer stride remains `Qs_1=32`.
- No binary or Arena submission was produced.
