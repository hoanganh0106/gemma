...
type HalfLevels = DmTensor<f8e4m3, Chip, OutputClusters, SlidingOutputColumns, m![Lq, Qs % 128]>;
...
pub(crate) fn project_normalize_add(
    ...
    // Isolated graph probe. The default path remains P078; this branch is only
    // enabled when the package is launched with P108_HALF_GRAPH=1.
    if let Ok(stage) = std::env::var("P108_HALF_STAGE") {
        match stage.as_str() {
            "materialize" => {
                let _ = explicit_half_intermediate_probe(device, weight, &x);
            }
            "quant" => {
                let (_, x_half) = explicit_half_intermediate_probe(device, weight, &x);
                let _ = quantise_x_half(device, &x_half);
            }
            "contract" => {
                let _ = explicit_half_contract_chain_probe(device, weight, &x);
            }
            _ => panic!("unknown P108_HALF_STAGE={stage}"),
        }
        return;
    }
    ...
    let z_tail: TailDm<bf16> = z.to_dm(&mut device.tdma);
...
fn quantise_x_half(
    device: &mut Device,
    x: &DmTensor<bf16, Chip, OutputClusters, SlidingOutputColumns, m![Qs % 128]>,
) -> TrfTensor<f8e4m3, Chip, OutputClusters, SlidingOutputColumns, m![Lq], m![Qs % 128]> {
    let mut levels: HalfLevels = DmTensor::new();
    device.main
        .begin(x.view())
        .fetch::<m![1], m![Qs % 128]>()
        .fetch_cast::<f32>()
        ...
        .vector_init()
        .vector_intra_slice_tag(TagMode::Zero)
        .vector_narrow_split::<m![Qs / 4 % 32], m![Qs % 4]>()
        .vector_fp_binary(FpBinaryOp::MulF(FpMulAlu::Mul0), X_SCALE)
        .vector_widen_concat::<m![Qs / 8 % 16], m![Qs % 8]>()
        .vector_final()
        .cast::<f8e4m3, m![Qs % 8 # 32]>()
        .commit_trim::<m![Qs % 8]>()
        .commit_view(levels.view_mut().tile::<m![Lq], 1, m![Lq = 1 #{!} 2, Qs % 128]>(0));
    device.sub
        .begin(levels.view())
        .fetch::<m![Lq], m![Qs % 128]>()
        .collect::<m![Lq, Qs / 32 % 4], m![Qs % 32]>()
        .to_trf()
}

// Compile-only ownership probe: materialize Qs=128 tensors before contract.
// This is the required shape boundary for a real column split.
#[allow(dead_code)]
fn explicit_half_intermediate_probe(
    device: &mut Device,
    weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
    x: &DmTensor<bf16, Chip, OutputClusters, SlidingOutputColumns, m![Qs % 256]>,
) -> (
    DmTensor<f8e4m3, Chip, OutputClusters, SlidingOutputColumns, m![H % 120, Qs % 128]>,
    DmTensor<bf16, Chip, OutputClusters, SlidingOutputColumns, m![Qs % 128]>,
) {
    let weight_half = weight
        .view()
        .tile::<m![H, Qs], 128, m![H, Qs = 128 # 256]>(0)
        .to_dm(&mut device.tdma);
    let mut x_half: DmTensor<bf16, Chip, OutputClusters, SlidingOutputColumns, m![Qs % 128]> = DmTensor::new();
    x
        .view()
        .tile::<m![Qs % 256], 128, m![Qs = 128 # 256]>(0)
        .to_dm_view(&mut device.tdma, x_half.view_mut());
    (weight_half, x_half)
}

#[allow(dead_code)]
fn explicit_half_contract_chain_probe(
    device: &mut Device,
    weight: &HbmTensor<f8e4m3, Chip, m![H, Qs]>,
    x: &DmTensor<bf16, Chip, OutputClusters, SlidingOutputColumns, m![Qs % 256]>,
) -> Z {
    let (weight_half, x_half) = explicit_half_intermediate_probe(device, weight, x);
    let x_trf_half = quantise_x_half(device, &x_half);
    column_half_contract_probe(device, &weight_half, &x_trf_half)
}

// Compile-only probe for the column-split family. It is intentionally unused: the probe checks
// whether one Qs half can contract into the original full H/120 output mapping before any full
// kernel integration is attempted.
#[allow(dead_code)]
fn column_half_contract_probe(
    device: &mut Device,
    weight: &DmTensor<f8e4m3, Chip, OutputClusters, SlidingOutputColumns, m![H % 120, Qs % 128]>,
    x: &TrfTensor<f8e4m3, Chip, OutputClusters, SlidingOutputColumns, m![Lq], m![Qs % 128]>,
) -> DmTensor<bf16, Chip, OutputClusters, Rows, m![H % 120]> {
    let mut out: Z = DmTensor::new();
    device.main
        .begin(weight.view())
        .fetch::<m![H % 120, Qs / 32 % 4], m![Qs % 32]>()
        .collect::<m![H % 120, Qs / 32 % 4], m![Qs % 32]>()
        .contract_outer::<m![H % 120, Qs / 64 % 2], m![Qs % 64], _, _, _>(x)
        .contract_packet::<m![1]>()
        .contract_time::<m![H % 120]>()
        .contract_lane::<m![H % 120, Lq], m![1 # 8]>(LaneMode::Sequential)
        .vector_init()
...
// Compile-only accumulation gate: two Qs-half results must be addable without
// changing the existing full H/120 ownership.
#[allow(dead_code)]
fn column_half_accumulate_probe(
    device: &mut Device,
    a: &DmTensor<bf16, Chip, OutputClusters, Rows, m![H % 120]>,
    b: &DmTensor<bf16, Chip, OutputClusters, Rows, m![H % 120]>,
) -> DmTensor<bf16, Chip, OutputClusters, Rows, m![H % 120]> {
    let mut out: Z = DmTensor::new();
    let b_vrf: VrfTensor<f32, Chip, OutputClusters, Rows, m![H % 120]> = device
        .sub
        .begin(b.view())
        .fetch::<m![1], m![H % 120]>()
        .fetch_cast::<f32>()
    ...
    device
        ...
        .vector_init()
        .vector_intra_slice_tag(TagMode::Zero)
        .vector_clip(ClipBinaryOpF32::Add, &b_vrf)
        .vector_final()
        .cast::<bf16, m![H % 8 # 16]>()
        .commit_trim::<m![H % 8]>()
        .commit_view(out.view_mut());
...
// End-to-end compile gate for the two-column producer/consumer chain.
#[allow(dead_code)]
fn column_split_chain_probe(
    device: &mut Device,
    weight0: &DmTensor<f8e4m3, Chip, OutputClusters, SlidingOutputColumns, m![H % 120, Qs % 128]>,
    x0: &TrfTensor<f8e4m3, Chip, OutputClusters, SlidingOutputColumns, m![Lq], m![Qs % 128]>,
    weight1: &DmTensor<f8e4m3, Chip, OutputClusters, SlidingOutputColumns, m![H % 120, Qs % 128]>,
    x1: &TrfTensor<f8e4m3, Chip, OutputClusters, SlidingOutputColumns, m![Lq], m![Qs % 128]>,
) -> DmTensor<bf16, Chip, OutputClusters, Rows, m![H % 120]> {
    let partial0 = column_half_contract_probe(device, weight0, x0);
    let partial1 = column_half_contract_probe(device, weight1, x1);
    column_half_accumulate_probe(device, &partial0, &partial1)
}
