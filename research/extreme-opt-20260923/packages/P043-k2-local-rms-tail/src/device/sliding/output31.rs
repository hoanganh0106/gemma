...
pub(crate) fn project_normalize_add(
    ...
    // Compute the RMS partials in the native projection layout.  The P041 tail
    // redistribution can then run in parallel with this Main work instead of
    // being a dependency of the RMS chain.
    ...
    let local_scale_dm: LocalDm<bf16> = weight_scale.to_dm(&mut device.tdma);
    let local_scale: LocalVrf = device
        .sub
        .begin(local_scale_dm.view())
        .fetch::<m![1], m![H % 120]>()
        .fetch_cast::<f32>()
    ...
    // Preserve P041's fast direct redistribution for the final pass.
    let z_tail: TailDm<bf16> = z.to_dm(&mut device.tdma);

    // Divide each local sum by global H.  The peer-cluster sum below therefore
    // yields the exact global mean square used by P041.
    let local_partial: MeanDm = device
        .main
        .begin(z.view())
        .fetch::<m![1], m![H % 120]>()
        .fetch_cast::<f32>()
        ...
        .vector_fp_binary(FpBinaryOp::MulF(FpMulAlu::Mul0), &local_scale)
        .vector_stash()
    ...
    let local_mean: DmTensor<f32, Chip, OutputClusters, m![Vr, 1 # 16], m![1 # 8]> = device
        .main
        .begin(local_partial.view())
        .fetch::<m![1], m![1 # 8]>()
    ...
    let peer: MeanVrf = device
        .sub
        .begin(peer_mean.view())
        .fetch::<m![1], m![1 # 8]>()
        .collect::<m![1], m![1 # 8]>()
        .vector_init()
        .vector_intra_slice_tag(TagMode::Zero)
        ...
        .vector_final()
        .to_vrf();
    let inv_local: MeanVrf = device
        .main
        .begin(local_mean.view())
        .fetch::<m![1], m![1 # 8]>()
        .collect::<m![1], m![1 # 8]>()
        .vector_init()
        .vector_intra_slice_tag(TagMode::Zero)
    ...
    // C17 proves the scalar reduction layout; P043 keeps only cluster 0 and
    // reinterprets its repeated scalar values in P041's 240-row tail layout.
    let inv_rms_vrf: VrfTensor<f32, Chip, Vc, Tail, m![1 # 8]> = unsafe { inv_local.reshape() };
