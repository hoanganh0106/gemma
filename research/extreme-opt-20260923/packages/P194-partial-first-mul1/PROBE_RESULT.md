...
- P194 changed only the first partial RMS square multiply (`scale`) from `Mul0` to `Mul1`, leaving `Stash` on `Mul1`.
- Clean SDK 0.8.1 build reached VISA but rejected the schedule: `14 is not available for op Binary(MulF)` at output31.rs:215.
