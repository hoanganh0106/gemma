...
- P196 replaced the final `vector_clip(ClipBinaryOpF32::Add, &residual)` with `vector_fp_binary(AddF, &residual)`.
- Clean SDK 0.8.1 build rejected the operation because `vector_fp_binary` is unavailable on the post-cast `VectorTensor` at output31.rs:259.
