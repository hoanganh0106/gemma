...
## Build probe: transpose grouping by 2 rows

- Source change: final output transpose/trim changed from `H % 120 / 4` with `H % 120 % 4` to `H % 120 / 2` with `H % 120 % 2`.
- Clean SDK 0.8.1 build failed in VISA before schedule generation: `commit_trim output packet must be one of [8, 16, 24, 32] bytes, got 4`.
- No binary was produced and no Arena submission was made.
