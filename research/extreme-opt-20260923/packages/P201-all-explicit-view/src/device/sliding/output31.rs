...
pub(crate) fn project_normalize_add(
    ...
    let mut residual_dm: TailDm<bf16> = DmTensor::new();
    residual_hbm.view().to_dm_view(&mut device.tdma, residual_dm.view_mut());
    ...
    let mut scale_dm: TailDm<bf16> = DmTensor::new();
    weight_scale.view().to_dm_view(&mut device.tdma, scale_dm.view_mut());
    ...
    let mut rms_dm: TailDm<bf16> = DmTensor::new();
    rms_weight.view().to_dm_view(&mut device.tdma, rms_dm.view_mut());
