...
fn normalize_quantize(
    ...
    // P062: replace the no-op WARM-UP with a real, dependency-free Sub fetch of rms_weight. The
    // first Sub command of this kernel still pays the ~10x issuer stall, but now it does useful work
    // (the rms_weight -> weight_vrf load) instead of a dummy StoVrf. This keeps the stall off the
    // critical chain while removing the wasted 267-cycle command and letting the DMA/Main overlap
    // begin earlier.
