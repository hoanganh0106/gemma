...
pub(crate) fn apply_rope(
    ...
    let cos_vrf: VrfTensor<f32, Chip, Cluster, Slice, m![Ds]> = device
        .sub
    ...
    let sin_vrf: VrfTensor<f32, Chip, Cluster, Slice, m![Ds]> = device
        .sub
    ...
    let q_sin: DmTensor<f32, Chip, Cluster, Slice, m![Gs, Ds]> = device
        .main
    ...
    let q_sin_vrf: VrfTensor<f32, Chip, Cluster, Slice, m![Gs, Ds]> = device
        .sub
        .begin(q_sin.view())
    ...
    let result_q: DmTensor<bf16, Chip, Cluster, Slice, m![Gs, Ds]> = device
        .main
        .begin(q.view())
        .fetch::<m![Gs, Ds / 16], m![Ds % 16]>()
        .fetch_cast::<f32>()
    ...
    let k_sin: DmTensor<f32, Chip, Cluster, Slice, m![Ds]> = device
        .main
    ...
    let k_sin_vrf: VrfTensor<f32, Chip, Cluster, Slice, m![Ds]> = device
        .sub
        .begin(k_sin.view())
        .fetch::<m![Ds / 8], m![Ds % 8]>()
        .collect::<m![Ds / 8], m![Ds % 8]>()
    ...
    let result_k: DmTensor<bf16, Chip, Cluster, Slice, m![Ds]> = device
        .main
        .begin(k.view())
        .fetch::<m![Ds / 16], m![Ds % 16]>()
        .fetch_cast::<f32>()
