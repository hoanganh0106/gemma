...
## Compile probe — repartition `Vc 1#4 + Tail 1#4`

- Source change: `Vc = 1#2, Tail = 1#8` -> `Vc = 1#4, Tail = 1#4`.
- Clean SDK 0.8.1 build reached MIR and rejected the mapping: `DmTensor T240's cluster extent 4 does not match the device config (cluster = 2)`.
- The current device config fixes both cluster and slice ownership (`cluster = 2`, `slice = 256`); repartitioning Tail cannot reduce final store traffic. No binary or Arena submission was produced.
