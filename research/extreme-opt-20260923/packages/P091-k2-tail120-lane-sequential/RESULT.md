...
## P091 sequential lane probe

P091 changes only the partial projection contract from `LaneMode::Interleaved` to `LaneMode::Sequential`. The SDK 0.8.1 release build completed, but runtime graph initialization failed before schedule generation with `gather src residue has no live target axis`. The sequential lane assignment breaks the downstream gather ownership/layout invariant. Rejected before Arena.
