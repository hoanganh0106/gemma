...
pub(crate) type WeightA =
    DmTensor<f8e4m3, Chip, OutputClusters, SlidingOutputColumns, m![H % 120 = 112, Qs % 256]>;

pub(crate) fn contract_a(
    device: &mut Device,
    weight: &WeightA,
    x_trf: &XTrf,
    z: &mut Z,
    ...
    device.main
        .begin(weight.view())
        .fetch::<m![H % 120 = 112, Qs / 32 % 8], m![Qs % 32]>()
        .collect::<m![H % 120 = 112, Qs / 32 % 8], m![Qs % 32]>()
        .contract_outer::<m![H % 120 = 112, Qs / 64 % 4], m![Qs % 64], _, _, _>(x_trf)
        .contract_packet::<m![1]>()
        .contract_time::<m![H % 120 = 112]>()
        .contract_lane::<m![H % 120 = 112, Lq], m![1 # 8]>(LaneMode::Sequential)
        .vector_init()
        .vector_intra_slice_tag(TagMode::Zero)
        .vector_narrow_trim::<m![1 # 4]>()
        .vector_intra_slice_reduce::<Lq, m![H % 120 = 112], m![1 # 4]>(IntraSliceReduceOpF32::Add)
        .vector_widen_pad::<m![1 # 8]>()
        .vector_inter_slice_reduce::<Rows, m![H % 120 = 112]>(InterSliceReduceOpF32::Add)
        .vector_final()
        .cast::<bf16, m![1 # 16]>()
        .transpose::<m![H % 120 = 112 / 4], m![H % 120 = 112 % 4 # 16]>()
        .commit_trim::<m![H % 120 = 112 % 4]>()
        .commit_view(
            z.view_mut().tile::<m![H % 120], 112, m![H % 120 = 112 #{!} 120]>(row_offset),
        );
}

pub(crate) type WeightB =
    DmTensor<f8e4m3, Chip, OutputClusters, SlidingOutputColumns, m![H % 120 = 8, Qs % 256]>;

pub(crate) fn contract_b(
    device: &mut Device,
    weight: &WeightB,
    x_trf: &XTrf,
    z: &mut Z,
    ...
    device.main
        .begin(weight.view())
        .fetch::<m![H % 120 = 8, Qs / 32 % 8], m![Qs % 32]>()
        .collect::<m![H % 120 = 8, Qs / 32 % 8], m![Qs % 32]>()
        .contract_outer::<m![H % 120 = 8, Qs / 64 % 4], m![Qs % 64], _, _, _>(x_trf)
        .contract_packet::<m![1]>()
        .contract_time::<m![H % 120 = 8]>()
        .contract_lane::<m![H % 120 = 8, Lq], m![1 # 8]>(LaneMode::Sequential)
        .vector_init()
        .vector_intra_slice_tag(TagMode::Zero)
        .vector_narrow_trim::<m![1 # 4]>()
        .vector_intra_slice_reduce::<Lq, m![H % 120 = 8], m![1 # 4]>(IntraSliceReduceOpF32::Add)
        .vector_widen_pad::<m![1 # 8]>()
        .vector_inter_slice_reduce::<Rows, m![H % 120 = 8]>(InterSliceReduceOpF32::Add)
        .vector_final()
        .cast::<bf16, m![1 # 16]>()
        .transpose::<m![H % 120 = 8 / 4], m![H % 120 = 8 % 4 # 16]>()
        .commit_trim::<m![H % 120 = 8 % 4]>()
        .commit_view(
            z.view_mut().tile::<m![H % 120], 8, m![H % 120 = 8 #{!} 120]>(row_offset),
        );
}
...
pub(crate) fn project_normalize_add(
    ...
    // Large-first row split: keep the final contraction small while TDMA streams the tail rows.
    let weight_a: WeightA = weight
        .view()
        .tile::<m![H % 120], 112, m![H / 120, H % 120 = 112 # 120, Qs]>(0)
        .to_dm(&mut device.tdma);
    let weight_b: WeightB = weight
        .view()
        .tile::<m![H % 120], 8, m![H / 120, H % 120 = 8 # 120, Qs]>(112)
        .to_dm(&mut device.tdma);
    ...
    // 16 * (weight @ x), bf16. Each chunk commits directly into its rows of z.
    let mut z: Z = DmTensor::new();
    contract_a(device, &weight_a, &x_trf, &mut z, 0);
    contract_b(device, &weight_b, &x_trf, &mut z, 112);
