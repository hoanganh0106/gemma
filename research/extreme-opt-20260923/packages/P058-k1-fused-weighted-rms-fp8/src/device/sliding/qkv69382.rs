...
pub(crate) fn normalize_native_input(
    ...
    let rms: DmTensor<f32, Chip, QkvDualQueryCluster, Copies, m![1 # 8]> = ctx
        .main
        .begin(mean.view())
        .fetch::<m![1], m![1 # 8]>()
        .collect::<m![1], m![1 # 8]>()
    ...
    let rms: DmTensor<f32, Chip, QkvDualQueryCluster, Shards, m![1 # 8]> =
        unsafe { rms.reshape() };
    let rms_vrf: VrfTensor<f32, Chip, QkvDualQueryCluster, Shards, m![1 # 8]> = ctx
        .sub
        .begin(rms.view())
        .fetch::<m![1], m![1 # 8]>()
        .collect::<m![1], m![1 # 8]>()
    ...
    let weight_vrf: VrfTensor<f32, Chip, QkvDualQueryCluster, Shards, m![H % 120]> = ctx
        ...
        .collect::<m![H / 8 % 15], m![H % 8]>()
        .to_vrf();
    // Fuse the weighted normalization and one-term FP8 quantization so the normalized
    // activation never needs a separate DM materialization/readback. input_rms_weight
    // remains on the live arithmetic path before the projection operand is created.
