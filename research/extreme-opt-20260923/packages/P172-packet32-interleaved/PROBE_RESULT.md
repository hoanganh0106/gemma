...
## Compile probe — packet32 interleaved lane

- Source change: kept packet `m![1 # 8]`, changed `LaneMode::Sequential` to `LaneMode::Interleaved`.
- Clean SDK 0.8.1 build reached MIR validation and rejected the mapping:
  `contract_lane (Interleaved): OutPacket mismatch. Expected: Lq # 8, got: 1 # 8`.
