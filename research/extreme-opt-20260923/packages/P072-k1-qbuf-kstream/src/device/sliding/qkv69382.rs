...
pub(crate) type RawQueryWeight =
    DmTensor<f8e4m3, Chip, HeadCluster, QueryStripe, m![Ds / 8 % 8, H]>;
pub(crate) type RawKvWeight =
    DmTensor<f8e4m3, Chip, HeadCluster, m![Ns % 4, Ds / 32, Ds % 4, Ds / 4 % 2], m![Ds / 8 % 4, H]>;
pub(crate) type RawKvChunk =
    DmTensor<f8e4m3, Chip, HeadCluster, m![Ns % 4, Ds = 64], m![1, H]>;
...
pub(crate) fn load_query_weight(
    ctx: &mut Device,
...
/// Load only 64 output rows per KV head. Across the four heads owned by one
/// cluster this is exactly 256 rows, so each physical slice owns one row.
/// Keeping this allocation at one quarter of the full K matrix lets the
/// scheduler place the first K DMA while Q is still consuming MAIN.
pub(crate) fn load_kv_chunk(
    ctx: &mut Device,
    weight: &HbmTensor<f8e4m3, Chip, m![Ps, H]>,
    offset: usize,
) -> RawKvChunk {
    let mut store: RawKvChunk = DmTensor::new();
    load_kv_chunk_into(ctx, weight, offset, &mut store);
    store
}

fn load_kv_chunk_into(
    ctx: &mut Device,
    weight: &HbmTensor<f8e4m3, Chip, m![Ps, H]>,
    offset: usize,
    store: &mut RawKvChunk,
) {
    let weight_view: HbmTensorView<'_, f8e4m3, Chip, m![Ns, Ds, H]> =
        unsafe { weight.view().reshape() };
    let chunk = weight_view.tile::<m![Ds], 64, m![Ns, Ds = 64 # 256, H]>(offset);
    chunk.to_dm_view(&mut ctx.tdma, store.view_mut());
}

fn project_kv_chunk_into(
    ctx: &mut Device,
    x_trf: &NativeTrf,
    weight: DmTensorView<'_, f8e4m3, Chip, HeadCluster, m![Ns % 4, Ds = 64], m![1, H]>,
    offset: usize,
    out: &mut KvHeadTensor,
) {
    let x_rows: &TrfTensor<f8e4m3, Chip, HeadCluster, m![Ns % 4, Ds = 64], m![1], m![H]> =
        unsafe { std::mem::transmute(x_trf) };
    let sums: DmTensor<f32, Chip, HeadCluster, m![Ns % 4, Ds = 64], m![1, 1 # 8]> = ctx
        .main
        .begin(weight)
        .fetch::<m![1, H / 32], m![H % 32]>()
        .collect::<m![1, H / 32], m![H % 32]>()
        .contract_outer::<m![1, H / 32], m![H % 32], _, _, _>(x_rows)
        .contract_packet::<m![1]>()
        .contract_time::<m![1]>()
    ...
    ctx.main
        .begin(sums.view())
        .fetch::<m![1], m![1 # 8]>()
        .switch::<KvHeads, m![Ds = 64]>(SwitchConfig::Broadcast1 {
            slice1: 64,
            slice0: 1,
        })
        .collect::<m![Ds = 64], m![1 # 8]>()
        .cast::<bf16, m![1 # 16]>()
        .transpose::<m![Ds = 64 / 4], m![Ds = 64 % 4 # 16]>()
        .commit_trim::<m![Ds = 64 % 4]>()
        .commit_view(out.view_mut().tile::<m![Ds], 64, m![Ds = 64 #{!} 256]>(offset));
}

pub(crate) fn project_key_streamed(
    ctx: &mut Device,
    x_trf: &NativeTrf,
    weight: &HbmTensor<f8e4m3, Chip, m![Ps, H]>,
    first_chunk: &RawKvChunk,
    query_buffer: &mut RawQueryWeight,
) -> KvHeadTensor {
    let mut out: KvHeadTensor = DmTensor::new();

    // Keep chunk 0 independently preloadable. The remaining chunks deliberately
    // reuse the storage that held Q weights.  Q has already consumed this buffer
    // before this function is called, so these writes create a real Q-read ->
    // K-write dependency in the scheduler instead of relying on source call order.
    project_kv_chunk_into(ctx, x_trf, first_chunk.view(), 0, &mut out);
    for i in 0..3 {
        let offset = (i + 1) * 64;
        let weight_view: HbmTensorView<'_, f8e4m3, Chip, m![Ns, Ds, H]> =
            unsafe { weight.view().reshape() };
        let chunk = weight_view.tile::<m![Ds], 64, m![Ns, Ds = 64 # 256, H]>(offset);
        // One eighth of RawQueryWeight has exactly the same wire volume as one
        // 64-row KV chunk: 256 physical slices x H elements per head cluster.
        // Cut that region first, then reshape it.  This preserves the Q
        // allocation/storage identity while avoiding a live-axis mismatch.
        chunk.to_dm_view(
            &mut ctx.tdma,
            query_buffer
                .view_mut()
                .tile::<m![Ds / 8 % 8], 1, m![Ds / 8 % 8 = 1 #{!} 8, H]>(0),
        );

        let buffer: DmTensorView<
            '_,
            f8e4m3,
            Chip,
            HeadCluster,
        ...
        project_kv_chunk_into(ctx, x_trf, buffer, offset, &mut out);
    }
    out
}
...
fn project_one_kv_matrix_raw_hbm(
    ctx: &mut Device,
    x_trf: &NativeTrf,
    weight: &HbmTensor<f8e4m3, Chip, m![Ps, H]>,
) -> KvHeadTensor {
    let weight = load_kv_weight(ctx, weight);
    project_one_kv_matrix_raw(ctx, x_trf, &weight)
}
...
pub(crate) fn project_key_value(
    ...
    let k = project_one_kv_matrix_raw_hbm(ctx, x_trf, k_weight);
    let v_raw = project_one_kv_matrix_raw_hbm(ctx, x_trf, v_weight);
    let v = normalize_value_project_scaled(ctx, &v_raw, v_weight_scale);
    (k, v)
}

pub(crate) fn project_key_loaded(
    ctx: &mut Device,
    x_trf: &NativeTrf,
    k_weight: &RawKvWeight,
) -> KvHeadTensor {
