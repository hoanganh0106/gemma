...
pub fn sliding_project_qkv(
    ...
    let mut q_weight = sliding::qkv69382::load_query_weight(device, q_weight);
    let k_buffer = sliding::qkv69382::load_kv_chunk(device, k_weight, 0);
    let q = sliding::qkv69382::project_query(device, &x, &q_weight, q_weight_scale);
    let k = sliding::qkv69382::project_key_streamed(device, &x, k_weight, &k_buffer, &mut q_weight);
