...
type RawKvWeight =
    DmTensor<f8e4m3, Chip, HeadCluster, KvStripe, m![Ds / 8 % 4, H]>;
...
pub(crate) fn new_query_weight_buffer() -> RawQueryWeight {
    DmTensor::new()
}
...
fn project_one_kv_matrix_raw_dm(
    ctx: &mut Device,
    x_trf: &NativeTrf,
    weight: DmTensorView<'_, f8e4m3, Chip, HeadCluster, KvStripe, m![Ds / 8 % 4, H]>,
) -> KvHeadTensor {
    let x_trf: &TrfTensor<f8e4m3, Chip, HeadCluster, KvStripe, m![1], m![H]> =
        unsafe { std::mem::transmute(x_trf) };
    let sums: DmTensor<f32, Chip, HeadCluster, KvStripe, m![Ds / 8 % 4, 1 # 8]> = ctx
...
fn project_one_kv_matrix_raw(
    ...
    let weight_view: HbmTensorView<'_, f8e4m3, Chip, m![Ns, Ds, H]> =
        unsafe { weight.view().reshape() };
    let weight_dm: DmTensor<f8e4m3, Chip, HeadCluster, KvStripe, m![Ds / 8 % 4, H]> =
        weight_view.to_dm(&mut ctx.tdma);
    project_one_kv_matrix_raw_dm(ctx, x_trf, weight_dm.view())
}

pub(crate) fn project_key_from_query_buffer(
    ctx: &mut Device,
    x_trf: &NativeTrf,
    weight: &HbmTensor<f8e4m3, Chip, m![Ps, H]>,
    ...
    // K occupies exactly half of the Q-weight allocation.  Loading K into this
    // storage and consuming it before Q overwrites the same allocation creates
    // a real K-read -> Q-write hazard for the scheduler.
    let key_buffer: &mut RawKvWeight = unsafe { std::mem::transmute(query_buffer) };
    weight_view.to_dm_view(&mut ctx.tdma, key_buffer.view_mut());

    project_one_kv_matrix_raw_dm(ctx, x_trf, key_buffer.view())
}
