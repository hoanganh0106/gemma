...
pub(crate) fn project_normalize_add(
    ...
    // P076 mechanism probe: finish the inverse RMS in the gather pass.  The
    // gather already has all 16 partials, so its scalar result need not be
    // committed to DM and fetched by a second Main pass.
