...
pub(crate) fn project_query(
    ...
    let sums: DmTensor<f32, Chip, HeadCluster, QueryStripe, m![Ds / 8 % 8, 1 # 8]> = ctx
        ...
        .fetch::<m![Ds / 8 % 8, H / 64], m![H % 64]>()
        .collect::<m![Ds / 8 % 8, H / 64], m![H % 64]>()
        .contract_outer::<m![Ds / 8 % 8, H / 64], m![H % 64], _, _, _>(x_rows)
        .contract_packet::<m![1]>()
...
fn project_one_kv_matrix_scaled(
    ...
    let sums: DmTensor<f32, Chip, HeadCluster, KvStripe, m![Ds / 8 % 4, 1 # 8]> = ctx
        .main
        .begin(weight.view())
        .fetch::<m![Ds / 8 % 4, H / 64], m![H % 64]>()
        .collect::<m![Ds / 8 % 4, H / 64], m![H % 64]>()
        .contract_outer::<m![Ds / 8 % 4, H / 64], m![H % 64], _, _, _>(x_trf)
        .contract_packet::<m![1]>()
