...
pub(crate) type WeightA =
    DmTensor<f8e4m3, Chip, OutputClusters, SlidingOutputColumns, m![H % 120 = 96, Qs % 256]>;
...
pub(crate) fn contract_a(
    ...
    device.main
        ...
        .commit_view(
            z.view_mut().tile::<m![H % 120], 96, m![H % 120 = 96 #{!} 120]>(row_offset),
        );
}

pub(crate) type WeightB =
    DmTensor<f8e4m3, Chip, OutputClusters, SlidingOutputColumns, m![H % 120 = 24, Qs % 256]>;
...
pub(crate) fn contract_b(
    ...
    device.main
        ...
        .commit_view(
            z.view_mut().tile::<m![H % 120], 24, m![H % 120 = 24 #{!} 120]>(row_offset),
        );
}
...
pub(crate) fn project_normalize_add(
    ...
    contract_b(device, &weight_b, &x_trf, &mut z, 96);
