...
## Architecture family decision

The reshape family remains an architecture hypothesis, not a promoted
candidate. The hardware failure isolates the debt boundary: removing the
`z.to_dm` materialization lowers the producer schedule, but the existing
consumer ownership mapping cannot consume that producer layout. The next
discriminating experiment is a consumer mapping that preserves the full
H/120 owner while accepting the reshaped producer, before another Arena run.
