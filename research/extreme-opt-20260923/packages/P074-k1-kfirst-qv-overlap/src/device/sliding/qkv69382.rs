...
type KeyQueryStripe = m![Ns % 4, Gs = 1 # 2, Ds / 64, Ds % 8];
...
fn project_one_kv_matrix_from_query_tile(
    ctx: &mut Device,
    x_trf: &NativeTrf,
    weight: DmTensorView<'_, f8e4m3, Chip, HeadCluster, KeyQueryStripe, m![Ds / 8 % 8, H]>,
) -> KvHeadTensor {
    let x_rows: &TrfTensor<f8e4m3, Chip, HeadCluster, KeyQueryStripe, m![1], m![H]> =
        unsafe { std::mem::transmute(x_trf) };

    let sums: DmTensor<f32, Chip, HeadCluster, KeyQueryStripe, m![Ds / 8 % 8, 1 # 8]> = ctx
        .main
        .begin(weight)
        .fetch::<m![Ds / 8 % 8, H / 32], m![H % 32]>()
        .collect::<m![Ds / 8 % 8, H / 32], m![H % 32]>()
    ...
    let sorted: DmTensor<
        ...
        Chip,
        HeadCluster,
        m![Ns % 4, Gs = 1 # 2, Ds / 64, Ds / 8 % 8],
        m![Ds % 8],
    > = ctx
        .main
        .begin(sums.view())
        .fetch::<m![Ds / 8 % 8], m![1 # 8]>()
        .switch::<m![Ns % 4, Gs = 1 # 2, Ds / 64, Ds / 8 % 8], m![Ds % 8]>(
            SwitchConfig::InterTranspose {
                slice1: 8,
                slice0: 1,
                time0: 1,
    ...
    let heads: DmTensor<bf16, Chip, HeadCluster, m![Ns % 4, Dummy256 % 64], m![Ds]> = ctx.main
        .begin(sorted.view())
        .fetch::<m![1], m![Ds % 8]>()
        .switch::<m![Ns % 4, Dummy256 % 64], m![Ds / 8]>(SwitchConfig::Broadcast1 {
            slice1: 64,
            slice0: 1,
        })
...
pub(crate) fn project_key_from_query_buffer(
    ...
    weight_view.to_dm_view(
        ...
        query_buffer
            .view_mut()
            .slice_tile::<m![Gs], 1, m![Ns % 4, Gs = 1 #{!} 2, Ds / 64, Ds % 8]>(0),
    );

    project_one_kv_matrix_from_query_tile(
        ctx,
        x_trf,
        query_buffer
            .view()
