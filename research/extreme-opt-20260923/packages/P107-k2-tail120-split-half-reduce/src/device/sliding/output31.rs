...
pub(crate) type WeightHalf = DmTensor<f8e4m3, Chip, OutputClusters, SlidingOutputColumns, m![H % 120 = 60, Qs % 256]>;

/// Every one of the 120 rows of every row slice: the two levels summed inside the pass.
pub(crate) fn contract_tile(device: &mut Device, weight: &WeightHalf, x_trf: &XTrf, z: &mut Z, row_offset: usize) {
    device.main
        .begin(weight.view())
        ...
        .vector_inter_slice_reduce::<m![H % 120 = 60], m![H % 120 = 60]>(InterSliceReduceOpF32::Add)
        .vector_final()
        .cast::<bf16, m![1 # 16]>()
...
pub(crate) fn project_normalize_add(
    ...
    // Split each 120-row slice into two independent 60-row DMA commands.
    let weight_lo: WeightHalf = weight.view()
        .tile::<m![H % 120], 60, m![H / 120, H % 120 = 60 # 120, Qs]>(0)
        .to_dm(&mut device.tdma);
    let weight_hi: WeightHalf = weight.view()
    ...
    contract_tile(device, &weight_lo, &x_trf, &mut z, 0);
    contract_tile(device, &weight_hi, &x_trf, &mut z, 60);
