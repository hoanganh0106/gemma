...
pub(crate) fn project_normalize_add(
    ...
    // Residual is only consumed by the final pass. Keep this read late so the mutable HBM output
    // does not hold a DRAM-reuse dependency across the RMS critical path.
