...
pub(crate) fn contract_half(device: &mut Device, weight: &HalfWeight, x_trf: &HalfXTrf) -> HalfPartialVrf {
    device.main
        .begin(weight.view())
        .fetch::<m![H % 120, Qh / 32 % 4], m![Qh % 32]>()
        .collect::<m![H % 120, Qh / 32 % 4], m![Qh % 32]>()
        .contract_outer::<m![H % 120, Qh / 64 % 2], m![Qh % 64], _, _, _>(x_trf)
        .contract_packet::<m![1]>()
        .contract_time::<m![H % 120]>()
        .contract_lane::<m![H % 120, Lq], m![1 # 8]>(LaneMode::Sequential)
        .vector_init()
...
pub(crate) fn contract_half_add(device: &mut Device, weight: &HalfWeight, x_trf: &HalfXTrf, first: &HalfPartialVrf, z: &mut Z) {
    device.main
        .begin(weight.view())
        .fetch::<m![H % 120, Qh / 32 % 4], m![Qh % 32]>()
        .collect::<m![H % 120, Qh / 32 % 4], m![Qh % 32]>()
...
pub(crate) fn quantise_x_half(
    ...
    device.main
        ...
        .fetch_cast::<f32>()
        .collect::<m![Qh / 8 % 16], m![Qh % 8]>()
        .vector_init()
        .vector_intra_slice_tag(TagMode::Zero)
        .vector_narrow_split::<m![Qh / 4 % 32], m![Qh % 4]>()
        .vector_fp_binary(FpBinaryOp::MulF(FpMulAlu::Mul0), X_SCALE)
        .vector_widen_concat::<m![Qh / 8 % 16], m![Qh % 8]>()
        .vector_final()
        .cast::<f8e4m3, m![Qh % 8 # 32]>()
        .commit_trim::<m![Qh % 8]>()
        .commit_view(levels.view_mut().tile::<m![Lq], 1, m![Lq = 1 #{!} 2, Qh % 128]>(0));
    let q0_vrf: VrfTensor<f32, Chip, OutputClusters, HalfColumns, m![Lq = 1, Qh % 128]> = device
        .sub
        .begin(levels.view().tile::<m![Lq], 1, m![Lq = 1 # 2, Qh % 128]>(0))
        .fetch::<m![Lq = 1], m![Qh = 2048 % 128]>()
        .fetch_cast::<f32>()
        .collect::<m![Lq = 1, Qh / 8 % 16], m![Qh % 8]>()
        .to_vrf();
    device.main
        .begin(x.view())
        .fetch::<m![1], m![Qh = 2048 % 128]>()
        ...
        .cast::<f8e4m3, m![Qh % 8 # 32]>()
        .commit_trim::<m![Qh % 8]>()
        .commit_view(levels.view_mut().tile::<m![Lq], 1, m![Lq = 1 #{!} 2, Qh % 128]>(1));
    device.sub
        .begin(levels.view())
        .fetch::<m![Lq], m![Qh = 2048 % 128]>()
        .collect::<m![Lq, Qh / 32 % 4], m![Qh % 32]>()
        .to_trf()
}
...
pub(crate) fn project_normalize_add(
    ...
    // Load the first Q half and keep its FP32 projection partial in VRF.
