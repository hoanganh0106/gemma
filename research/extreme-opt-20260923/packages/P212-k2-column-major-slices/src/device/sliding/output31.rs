...
// Keep logical tiles, but put column groups before row groups in physical slices.
type SlidingOutputColumns = m![Qs / 256, H / 120 % 16];
type SlidingOutputRows = m![1 # 16, H / 120 % 16];
use crate::{Chip, EPS};
...
pub(crate) fn project_normalize_add(
    ...
    let x: DmTensor<bf16, Chip, OutputClusters, m![Ns, Gs, H / 120 % 16], m![Ds]> = x.to_dm(&mut device.tdma);
