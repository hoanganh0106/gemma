...
pub(crate) type QueryWeightBuffer =
...
pub(crate) fn new_query_weight_buffer() -> QueryWeightBuffer {
    DmTensor::new()
}

pub(crate) fn load_query_weight_into(
    ctx: &mut Device,
    weight: &HbmTensor<f8e4m3, Chip, m![Qs, H]>,
    store: &mut QueryWeightBuffer,
) {
    let weight_view: HbmTensorView<'_, f8e4m3, Chip, m![Ns, Gs, Ds, H]> =
        unsafe { weight.view().reshape() };
    weight_view.to_dm_view(&mut ctx.tdma, store.view_mut());
}

pub(crate) fn project_query(
    ctx: &mut Device,
    x_trf: &NativeTrf,
    weight_dm: &QueryWeightBuffer,
...
pub(crate) fn project_key_into_query_buffer(
    ctx: &mut Device,
    x_trf: &NativeTrf,
    weight: &HbmTensor<f8e4m3, Chip, m![Ps, H]>,
    store: &mut QueryWeightBuffer,
) -> KvHeadTensor {
    // K and Q both occupy the same 256 physical slices. K needs only four
    // weight rows per slice while Q needs eight, so park K in the first half
    // of each Q slice. Reusing the same DM allocation creates a real
    // read-before-overwrite dependency: Q's later DMA cannot clobber this
    // region until K's contraction has consumed it.
    let weight_view: HbmTensorView<'_, f8e4m3, Chip, m![Ns, Ds, H]> =
        unsafe { weight.view().reshape() };
    weight_view.to_dm_view(
        &mut ctx.tdma,
        store
            .view_mut()
            .tile::<m![Ds / 8 % 8], 4, m![Ds / 8 % 8 = 4 #{!} 8, H]>(0),
    );

    let weight = store
        .view()
        .tile::<m![Ds / 8 % 8], 4, m![Ds / 8 % 8 = 4 # 8, H]>(0);
    let x_trf: &TrfTensor<f8e4m3, Chip, HeadCluster, QueryStripe, m![1], m![H]> =
        unsafe { std::mem::transmute(x_trf) };
    let sums: DmTensor<
        f32,
        Chip,
    ...
    > = ctx
        .main
        .begin(weight)
        .fetch::<m![Ds / 8 % 8 = 4 # 8, H / 32], m![H % 32]>()
        .collect::<m![Ds / 8 % 8 = 4 # 8, H / 32], m![H % 32]>()
        .contract_outer::<m![Ds / 8 % 8 = 4 # 8, H / 32], m![H % 32], _, _, _>(x_trf)
        .contract_packet::<m![1]>()
        .contract_time::<m![Ds / 8 % 8 = 4 # 8]>()
        .contract_lane::<m![Ds / 8 % 8 = 4 # 8], m![1 # 8]>(LaneMode::Interleaved)
        .vector_init()
        .vector_intra_slice_tag(TagMode::Zero)
        .vector_narrow_trim::<m![1 # 4]>()
        .vector_widen_pad::<m![1 # 8]>()
        ...
        .commit_trim::<m![1 # 8]>()
        .commit();
    // Swap the 4 K result times with a 4-way slice factor, exactly like the
    // normal KV path. The Q stripe has an 8-way final slice factor, so split it
    // as 4 x 2 and keep the 2-way remainder in slice ownership.
    let sorted: DmTensor<
        bf16,
        Chip,
        HeadCluster,
        m![Ns % 4, Gs, Ds / 64, Ds / 8 % 8 = 4, Ds % 2],
        m![Ds % 4],
    > = ctx
        .main
        .begin(sums.view())
        .fetch::<m![Ds / 8 % 8 = 4 # 8], m![1 # 8]>()
        .switch::<m![Ns % 4, Gs, Ds / 64, Ds / 8 % 8 = 4, Ds % 2], m![Ds / 2 % 4 # 8]>(
            SwitchConfig::InterTranspose {
                slice1: 4,
                slice0: 2,
                time0: 1,
