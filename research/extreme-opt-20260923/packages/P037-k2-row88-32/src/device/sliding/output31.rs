...
pub(crate) type WeightA =
    DmTensor<f8e4m3, Chip, OutputClusters, SlidingOutputColumns, m![H % 120 = 88, Qs % 256]>;
...
pub(crate) fn contract_a(
    ...
    device.main
        .begin(weight.view())
        .fetch::<m![H % 120 = 88, Qs / 32 % 8], m![Qs % 32]>()
        .collect::<m![H % 120 = 88, Qs / 32 % 8], m![Qs % 32]>()
        .contract_outer::<m![H % 120 = 88, Qs / 64 % 4], m![Qs % 64], _, _, _>(x_trf)
        .contract_packet::<m![1]>()
        .contract_time::<m![H % 120 = 88]>()
        .contract_lane::<m![H % 120 = 88, Lq], m![1 # 8]>(LaneMode::Sequential)
        .vector_init()
        .vector_intra_slice_tag(TagMode::Zero)
        .vector_narrow_trim::<m![1 # 4]>()
        .vector_intra_slice_reduce::<Lq, m![H % 120 = 88], m![1 # 4]>(IntraSliceReduceOpF32::Add)
        ...
        .commit_view(
            z.view_mut().tile::<m![H % 120], 88, m![H % 120 = 88 #{!} 120]>(row_offset),
        );
}

pub(crate) type WeightB =
    DmTensor<f8e4m3, Chip, OutputClusters, SlidingOutputColumns, m![H % 120 = 32, Qs % 256]>;
...
pub(crate) fn contract_b(
    ...
    device.main
        .begin(weight.view())
        .fetch::<m![H % 120 = 32, Qs / 32 % 8], m![Qs % 32]>()
        .collect::<m![H % 120 = 32, Qs / 32 % 8], m![Qs % 32]>()
        .contract_outer::<m![H % 120 = 32, Qs / 64 % 4], m![Qs % 64], _, _, _>(x_trf)
        .contract_packet::<m![1]>()
        .contract_time::<m![H % 120 = 32]>()
        .contract_lane::<m![H % 120 = 32, Lq], m![1 # 8]>(LaneMode::Sequential)
        .vector_init()
        .vector_intra_slice_tag(TagMode::Zero)
        .vector_narrow_trim::<m![1 # 4]>()
        .vector_intra_slice_reduce::<Lq, m![H % 120 = 32], m![1 # 4]>(IntraSliceReduceOpF32::Add)
        .vector_widen_pad::<m![1 # 8]>()
        .vector_inter_slice_reduce::<Rows, m![H % 120 = 32]>(InterSliceReduceOpF32::Add)
        .vector_final()
        .cast::<bf16, m![1 # 16]>()
        .transpose::<m![H % 120 = 32 / 4], m![H % 120 = 32 % 4 # 16]>()
        .commit_trim::<m![H % 120 = 32 % 4]>()
        .commit_view(
            z.view_mut().tile::<m![H % 120], 32, m![H % 120 = 32 #{!} 120]>(row_offset),
        );
}
...
pub(crate) fn project_normalize_add(
    ...
    contract_b(device, &weight_b, &x_trf, &mut z, 88);
