...
type RawKvStripe = m![Ns % 4, Ds / 32, Ds % 4, Ds / 4 % 2];
type RawKvWeight = DmTensor<f8e4m3, Chip, HeadCluster, RawKvStripe, m![Ds / 8 % 4, H]>;
...
fn project_one_kv_matrix_raw(
    ...
    let x_trf: &TrfTensor<f8e4m3, Chip, HeadCluster, RawKvStripe, m![1], m![H]> =
        unsafe { std::mem::transmute(x_trf) };
    let sums: DmTensor<f32, Chip, HeadCluster, RawKvStripe, m![Ds / 8 % 4, 1 # 8]> = ctx
        .main
        .begin(weight.view())
        .fetch::<m![Ds / 8 % 4, H / 32], m![H % 32]>()
        .collect::<m![Ds / 8 % 4, H / 32], m![H % 32]>()
...
pub(crate) fn project_key_value(
    ...
    // Stage both long HBM weight transfers before K compute. The TDMA can fetch V while the
    // tensor unit contracts/sorts/broadcasts K, removing the avoidable gap before V's transfer.
    let k_weight = load_raw_kv_weight(ctx, k_weight);
    let v_weight = load_raw_kv_weight(ctx, v_weight);
...
fn root_mean_square<S: M>(
    ...
    let mean_square: DmTensor<f32, Chip, HeadCluster, S, m![1 # 8]> = ctx
        ...
        .commit_trim::<m![1 # 8]>()
        .commit();
    let rms: DmTensor<f32, Chip, HeadCluster, S, m![1 # 8]> = ctx
        .main
...
fn normalize_value_project_scaled(
    ...
    let mean_square: DmTensor<f32, Chip, HeadCluster, KvHeads, m![1 # 8]> = ctx
        ...
        .commit_trim::<m![1 # 8]>()
        .commit();
    let rms: DmTensor<f32, Chip, HeadCluster, KvHeads, m![1 # 8]> = ctx
        .main
        .begin(mean_square.view())
        .fetch::<m![1], m![1 # 8]>()
        .collect::<m![1], m![1 # 8]>()
...
pub(crate) fn apply_rope(
    ...
    // 0.8.1 requires the VRF operand to have the same physical partition as the stream.
    // Materialize the same 256-element RoPE pair directly in the KV-head mapping instead
    // of transmuting the query-head mapping (valid in 0.6 lowering, rejected in 0.8.1).
...
pub(crate) fn normalize_native_input(
    ...
    // Correctness anchor for 0.8.1: execute the official RMSNorm including the
    // elementwise input_rms_weight, then hand off through HBM into QKV69382's
    // native 8x32-shard mapping. Projection/head-local/RoPE machinery stays unchanged.
    type BaseCluster = crate::device::layout::Cluster;
    type BaseSlice = crate::device::layout::Slice;
    let x0: DmTensor<bf16, Chip, BaseCluster, BaseSlice, m![H]> = x.to_dm(&mut ctx.tdma);
    let normalized0 = crate::device::shared::rmsnorm::normalize(ctx, &x0, weight);
    let mut normalized_hbm: HbmTensor<bf16, Chip, m![H]> = HbmTensor::new();
    normalized0
        .view()
        .to_hbm_view(&mut ctx.tdma, normalized_hbm.view_mut());
    let normalized: DmTensor<bf16, Chip, QkvDualQueryCluster, Shards, m![H % 120]> =
        normalized_hbm.to_dm(&mut ctx.tdma);
