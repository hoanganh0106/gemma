...
) -> TrfTensor<bf16, Chip, UpGateClusters, DownSlices, m![1], m![T1, Dt, Xp]> {
    // x is already the reference BF16 GeGLU result. Preserve it without an FP8 high/residual split.
    let x: DmTensor<bf16, Chip, UpGateClusters, GroupsX, m![T1, Xd % 120]> = unsafe { x.reshape() };
    let gathered: DmTensor<bf16, Chip, UpGateClusters, DownSlices, m![T1, Xd]> = device
        .main
        .begin(x.view())
        ...
        .commit_trim::<m![Xd % 24]>()
        .commit();
    let gathered: DmTensor<bf16, Chip, UpGateClusters, DownSlices, m![T1, Dt, Xp]> = unsafe { gathered.reshape() };
    device.sub
        .begin(gathered.view())
