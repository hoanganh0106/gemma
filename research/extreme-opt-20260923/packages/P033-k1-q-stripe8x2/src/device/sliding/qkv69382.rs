...
pub(crate) fn project_query(
    ...
    type QueryStripe = m![Ns % 4, Gs, Ds / 128, Ds % 8, Ds / 8 % 2];
    ...
    let weight_dm: DmTensor<f8e4m3, Chip, HeadCluster, QueryStripe, m![Ds / 16 % 8, H]> =
    ...
    let sums: DmTensor<f32, Chip, HeadCluster, QueryStripe, m![Ds / 16 % 8, 1 # 8]> = ctx
        .main
        .begin(weight_dm.view())
        .fetch::<m![Ds / 16 % 8, H / 32], m![H % 32]>()
        .collect::<m![Ds / 16 % 8, H / 32], m![H % 32]>()
        .contract_outer::<m![Ds / 16 % 8, H / 32], m![H % 32], _, _, _>(x_rows)
        .contract_packet::<m![1]>()
        .contract_time::<m![Ds / 16 % 8]>()
        .contract_lane::<m![Ds / 16 % 8], m![1 # 8]>(LaneMode::Interleaved)
        .vector_init()
    ...
    let sorted: DmTensor<bf16, Chip, HeadCluster, m![Ns % 4, Gs, Ds / 128, Ds / 16 % 8, Ds / 8 % 2], m![Ds % 8]> =
        ctx.main
            .begin(sums.view())
            .fetch::<m![Ds / 16 % 8], m![1 # 8]>()
            .switch::<m![Ns % 4, Gs, Ds / 128, Ds / 16 % 8, Ds / 8 % 2], m![Ds % 8]>(
