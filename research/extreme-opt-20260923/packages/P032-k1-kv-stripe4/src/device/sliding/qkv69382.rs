...
fn project_one_kv_matrix_raw(
    ...
    // Stripe consecutive KV rows across the 4-way row ring. This preserves
    // head ownership but gives TDMA long contiguous HBM row runs instead of
    // four rows parked on one slice.
    type KvStripe = m![Ns % 4, Ds / 16, Ds % 4];
    let weight_view: HbmTensorView<'_, f8e4m3, Chip, m![Ns, Ds, H]> =
        unsafe { weight.view().reshape() };
    let weight: DmTensor<f8e4m3, Chip, HeadCluster, KvStripe, m![Ds / 4 % 4, H]> =
        weight_view.to_dm(&mut ctx.tdma);
    let x_trf: &TrfTensor<f8e4m3, Chip, HeadCluster, KvStripe, m![1], m![H]> =
        unsafe { std::mem::transmute(x_trf) };
    let sums: DmTensor<f32, Chip, HeadCluster, KvStripe, m![Ds / 4 % 4, 1 # 8]> = ctx
        .main
        .begin(weight.view())
        ...
        .contract_lane::<m![Ds / 4 % 4], m![1 # 8]>(LaneMode::Interleaved)
        .vector_init()
        .vector_intra_slice_tag(TagMode::Zero)
        .vector_narrow_trim::<m![1 # 4]>()
        .vector_widen_pad::<m![1 # 8]>()
    ...
    > = ctx
        ...
        .switch::<m![Ns % 4, Ds / 16, Ds / 4 % 4], m![Ds % 4]>(
            SwitchConfig::InterTranspose {
                slice1: 4,
                slice0: 1,
                time0: 1,
