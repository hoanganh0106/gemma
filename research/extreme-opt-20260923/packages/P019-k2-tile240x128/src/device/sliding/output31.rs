...
use crate::device::layout::OutputClusters;
type SlidingOutputColumns = m![H / 240 % 8, Qs / 128];
type SlidingOutputRows = m![H / 240 % 8, 1 # 32];
use crate::{Chip, EPS};
...
type Levels = DmTensor<f8e4m3, Chip, OutputClusters, SlidingOutputColumns, m![Lq, Qs % 128]>;
...
pub(crate) type XTrf = TrfTensor<f8e4m3, Chip, OutputClusters, SlidingOutputColumns, m![Lq], m![Qs % 128]>;
pub(crate) type Z = DmTensor<bf16, Chip, OutputClusters, Rows, m![H % 240]>;

pub(crate) type WeightTile = DmTensor<f8e4m3, Chip, OutputClusters, SlidingOutputColumns, m![H % 240, Qs % 128]>;
...
pub(crate) fn contract_tile(device: &mut Device, weight: &WeightTile, x_trf: &XTrf, z: &mut Z) {
    device.main
        ...
        .contract_outer::<m![H % 240, Qs / 64 % 2], m![Qs % 64], _, _, _>(x_trf)
        .contract_packet::<m![1]>()
        .contract_time::<m![H % 240]>()
        .contract_lane::<m![H % 240, Lq], m![1 # 32]>(LaneMode::Sequential)
        .vector_init()
        .vector_intra_slice_tag(TagMode::Zero)
        .vector_narrow_trim::<m![1 # 4]>()
        .vector_intra_slice_reduce::<Lq, m![H % 240], m![1 # 4]>(IntraSliceReduceOpF32::Add)
        .vector_widen_pad::<m![1 # 32]>()
        .vector_inter_slice_reduce::<Rows, m![H % 240]>(InterSliceReduceOpF32::Add)
        .vector_final()
        .cast::<bf16, m![1 # 16]>()
        .transpose::<m![H % 240 / 4], m![H % 240 % 4 # 16]>()
        .commit_trim::<m![H % 240 % 4]>()
...
pub(crate) fn quantise_x(
    ...
    let q0_vrf: VrfTensor<f32, Chip, OutputClusters, SlidingOutputColumns, m![Lq = 1, Qs % 128]> = device
        .sub
        .begin(levels.view().tile::<m![Lq], 1, m![Lq = 1 # 2, Qs % 128]>(0))
        .fetch::<m![Lq = 1], m![Qs % 128]>()
        .fetch_cast::<f32>()
        .collect::<m![Lq = 1, Qs / 8 % 16], m![Qs % 8]>()
        .to_vrf();
    device.main
        .begin(x.view())
        .fetch::<m![1], m![Qs % 128]>()
        ...
        .cast::<f8e4m3, m![Qs % 8 # 32]>()
        .commit_trim::<m![Qs % 8]>()
        .commit_view(levels.view_mut().tile::<m![Lq], 1, m![Lq = 1 #{!} 2, Qs % 128]>(1));
    device.sub
        .begin(levels.view())
        .fetch::<m![Lq], m![Qs % 128]>()
...
pub(crate) fn project_normalize_add(
    ...
    // The whole weight in ONE DMA command: 60 rows per slice, with 512 contiguous weight bytes per row.
    ...
    let x: DmTensor<bf16, Chip, OutputClusters, m![H / 240 % 8, Ns, Gs, Ds / 128], m![Ds % 128]> = x.to_dm(&mut device.tdma);
    let x: DmTensor<bf16, Chip, OutputClusters, SlidingOutputColumns, m![Qs % 128]> = unsafe { x.reshape() };
    let x_trf = quantise_x(device, &x);

    // 16 * (weight @ x), bf16, 60 rows per row slice.
    ...
    let partial: DmTensor<f32, Chip, Vc, Tail, m![1 # 32]> = device
        .main
        .begin(z_tail.view())
        .fetch::<m![1], m![H % 480]>()
        .fetch_cast::<f32>()
    ...
    // Eight values per slice, so the divider runs 8 times instead of once per channel.
    let inv_rms: VrfTensor<f32, Chip, Vc, m![1 # 32, Vr], m![1 # 32]> = device
        .main
        .begin(partial.view())
        .fetch::<m![1], m![1 # 32]>()
        .collect::<m![1], m![1 # 32]>()
        .vector_init()
        .vector_inter_slice_reduce::<m![1 # 32, Vr], m![1]>(InterSliceReduceOpF32::Add)
        .vector_intra_slice_tag(TagMode::Zero)
        .vector_narrow_trim::<m![1 # 4]>()
    ...
    let inv_rms_vrf: VrfTensor<f32, Chip, Vc, Tail, m![1 # 32]> = unsafe { inv_rms.reshape() };
