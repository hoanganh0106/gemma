...
- P195 changed only the second partial RMS square multiply (`Stash`) from `Mul1` to `Mul0`, leaving the first `scale` multiply on `Mul0`.
- Clean SDK 0.8.1 build reached VISA but rejected the schedule: `13 is not available for op Binary(MulF)` at output31.rs:215.
