...
type Vc = m![1 # 2];
type WeightColumns = m![H / 60 % 32, Qs / 512];
...
type Levels = DmTensor<f8e4m3, Chip, OutputClusters, WeightColumns, m![Lq, Qs % 512]>;
...
pub(crate) type XTrf = TrfTensor<f8e4m3, Chip, OutputClusters, WeightColumns, m![Lq], m![Qs % 512]>;
pub(crate) type Z = DmTensor<bf16, Chip, OutputClusters, Rows, m![H % 120]>;

pub(crate) type WeightTile = DmTensor<f8e4m3, Chip, OutputClusters, WeightColumns, m![H % 120, Qs % 512]>;
...
pub(crate) fn contract_tile(device: &mut Device, weight: &WeightTile, x_trf: &XTrf, z: &mut Z) {
    device.main
        .begin(weight.view())
        .fetch::<m![H % 120, Qs / 32 % 16], m![Qs % 32]>()
        .collect::<m![H % 120, Qs / 32 % 16], m![Qs % 32]>()
        .contract_outer::<m![H % 120, Qs / 32 % 16], m![Qs % 32], _, _, _>(x_trf)
        .contract_packet::<m![1]>()
...
pub(crate) fn quantise_x(
    device: &mut Device,
    x: &DmTensor<bf16, Chip, OutputClusters, WeightColumns, m![Qs % 512]>,
) -> XTrf {
    ...
    let q0_vrf: VrfTensor<f32, Chip, OutputClusters, WeightColumns, m![Lq = 1, Qs % 512]> = device
        .sub
...
pub(crate) fn project_normalize_add(
    ...
    let x: DmTensor<bf16, Chip, OutputClusters, WeightColumns, m![Qs % 512]> = unsafe { x.reshape() };
    let x_trf = quantise_x(device, &x);

    // Probe: issue the large weight DMA only after x materialization and quantization.
    // This tests whether the compiler can expose useful overlap at the critical path.
