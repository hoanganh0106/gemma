...
## Compile probe — weight layout `60x512` with P166 tail120

- Attempted to keep P166 tail/consumer ownership while changing only the projection producer to `Qs%512` weight and activation tiles.
- Build reached VISA lowering but rejected the reshape because the existing `SlidingOutputColumns` still carries `Qs%256`; requested `Qs%512` creates an ambiguous duplicate stride:
  `duplicate tag LabelStride ... Qs ... stride 256`.
- The family requires a coordinated column type change through the producer/consumer boundary; no binary or Arena submission was produced.

## Follow-up mapping probe

- Added local producer columns `H/60%32 x Qs/512` to satisfy the 256 element device slice invariant.
- VISA then rejected the reshape with an ambiguous duplicate `H` stride (`H` stride 60) against the existing `H%120` consumer layout.
- This confirms the `60x512` family cannot be isolated to the producer; it requires a full H-axis boundary migration. No binary or Arena submission was made.
