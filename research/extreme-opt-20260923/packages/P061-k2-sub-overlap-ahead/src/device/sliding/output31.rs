...
pub(crate) fn project_normalize_add(
    ...
    // P061: issue these Sub-TU fetches BEFORE contract_tile so the Sub tensor unit runs them in
    // parallel with the Main contraction. They depend only on residual_hbm / weight_scale /
    // rms_weight HBM tensors, none of which share a version chain with z or x_trf, so the reorder
    // is safe and the Main/Sub overlap is free critical-path time.
    ...
    // 16 * (weight @ x), bf16, 120 rows per row slice. Runs on Main while Sub finishes the three
    // tail fetches above.
