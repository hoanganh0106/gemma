...
axes![Rep8 = 8, Q4 = 4, Rep32 = 32, Ring8 = 8, T1 = 1, T2 = 2, Xh = 3840, Rep4 = 4, Ring64 = 64, Xg = 120, Xd = 7680, Ut = 60, Dt = 120, Xp = 64, C2 = 2, Lead4 = 4, Hg = 128, Pw = 64, Sb = 240, Sd = 480];
...
fn normalize_quantize(
    ...
    let _warm_vrf: VrfTensor<f32, Chip, UpGateClusters, Pieces, m![1 # 8]> = device
        .sub
        .begin(warm.view())
        .fetch::<m![1], m![1 # 8]>()
        .collect::<m![1], m![1 # 8]>()
    ...
    let x: DmTensor<bf16, Chip, UpGateClusters, Loaded, m![H % 480]> = x.to_dm(&mut device.tdma);
    let x: DmTensor<bf16, Chip, UpGateClusters, Pieces, m![H % 480]> = unsafe { x.reshape() };

    let mean_square: DmTensor<f32, Chip, UpGateClusters, Pieces, m![1 # 8]> = device
        .main
        .begin(x.view())
        ...
        .commit_trim::<m![1 # 8]>()
        .commit();
    let rms: DmTensor<f32, Chip, UpGateClusters, Gathered, m![1 # 8]> = device
        .main
    ...
    let weight_dm: DmTensor<bf16, Chip, UpGateClusters, Loaded, m![H % 480]> = rms_weight.to_dm(&mut device.tdma);
    let weight_dm: DmTensor<bf16, Chip, UpGateClusters, Pieces, m![H % 480]> = unsafe { weight_dm.reshape() };
    let weight_vrf: VrfTensor<f32, Chip, UpGateClusters, Pieces, m![H % 480]> = device
        .sub
        ...
        .collect::<m![H / 8 % 60], m![H % 8]>()
        .to_vrf();
    let rms_vrf: VrfTensor<f32, Chip, UpGateClusters, Pieces, m![1 # 8]> = device
        .sub
        .begin(rms.view())
        .fetch::<m![1], m![1 # 8]>()
        .collect::<m![1], m![1 # 8]>()
        .to_vrf();
    let normalized: DmTensor<bf16, Chip, UpGateClusters, Pieces, m![H % 480]> = device
        .main
        .begin(x.view())
        .fetch::<m![H / 16 % 30], m![H % 16]>()
        .fetch_cast::<f32>()
    ...
    let q1_vrf: VrfTensor<f32, Chip, UpGateClusters, PiecesX, m![T2 = 1, Xh % 480]> = device
        .sub
    ...
    let spread: DmTensor<f8e4m3, Chip, UpGateClusters, m![Rep8, 4, Xh / 480], m![Q4, T2, Xh % 480]> = device
        .main
        .begin(q.view())
        .fetch::<m![1], m![T2, Xh % 480]>()
        .switch::<m![Rep8, 4, Xh / 480], m![1, Q4]>(SwitchConfig::Broadcast1 { slice1: 4, slice0: 8 })
    ...
    let gathered: DmTensor<f8e4m3, Chip, UpGateClusters, m![Rep8, 4, 8], m![Q4 = 1, T2, Xh]> = device
        .main
        .begin(spread.view().tile::<m![Q4], 1, m![Q4 = 1 # 4, T2, Xh % 480]>(0))
        .fetch::<m![Q4 = 1, T2], m![Xh % 480]>()
        .switch::<m![Rep8, 4, 8], m![Q4 = 1, T2, Xh / 480]>(SwitchConfig::Broadcast1 { slice1: 8, slice0: 1 })
...
macro_rules! block_sums_all {
    ($device:ident, $w:ident, $x_trf:ident) => {{
        let packed: DmTensor<f4e2m1, Chip, UpGateClusters, RowSlices, m![L % 30, H]> = $w.to_dm(&mut $device.tdma);
        let packed: DmTensor<f4e2m1, Chip, UpGateClusters, Gathered, m![L % 30, Ut, Xp]> =
            unsafe { packed.reshape() };
        let z: DmTensor<bf16, Chip, UpGateClusters, Gathered, m![L % 30, Ut, Xp / 16]> = $device
            .main
            .begin(packed.view())
...
fn scales_bf16(
    ...
    let s: DmTensor<f8e4m3, Chip, UpGateClusters, RowSlices, m![L % 30, H / 16]> = scale.to_dm(&mut device.tdma);
...
fn apply_scales(
    ...
    let s_trf: TrfTensor<bf16, Chip, UpGateClusters, Gathered, m![1], m![L % 30, Sb]> = device
        .sub
        .begin(s.view())
        .fetch::<m![L % 30, Sb / 16], m![Sb % 16]>()
        .collect::<m![L % 30, Sb / 16], m![Sb % 16]>()
        .to_trf();
    let y: DmTensor<f32, Chip, UpGateClusters, Gathered, m![L % 30]> = device
        .main
        .begin(z.view())
        .fetch::<m![L % 30, Sb / 16], m![Sb % 16]>()
        .collect::<m![L % 30, Sb / 16], m![Sb % 16]>()
...
fn gate_factor_vrf(
    ...
    let global_scale: DmTensor<f32, Chip, UpGateClusters, m![L / 120 % 64, 1 # 4], m![1 # 8]> =
        global_scale.to_dm(&mut device.tdma);
    ...
    let global_scale: DmTensor<f32, Chip, UpGateClusters, Groups, m![Lead4, 1 # 8]> = device
        .main
...
fn geglu(
    ...
    let gelu_vrf: VrfTensor<f32, Chip, UpGateClusters, Groups, m![L % 120]> = device
        .sub
        .begin(gate.view())
...
) -> TrfTensor<f8e4m3, Chip, UpGateClusters, DownSlices, m![1], m![T1, Dt, Xp]> {
    // Screening probe: keep the leading scaled f8 term and omit its residual term.
    // All model tensors/scales remain live; the final DOWN_GAIN undo is unchanged.
    let x: DmTensor<bf16, Chip, UpGateClusters, Groups, m![T1 = 1, Xg]> = unsafe { x.reshape() };
    let mut q: DmTensor<f8e4m3, Chip, UpGateClusters, Groups, m![T1, Xg]> = DmTensor::new();
    device.main
        .begin(x.view())
        .fetch::<m![T1 = 1, Xg / 8], m![Xg % 8]>()
        .fetch_cast::<f32>()
        .collect::<m![T1 = 1, Xg / 8], m![Xg % 8]>()
        .vector_init()
        .vector_intra_slice_tag(TagMode::Zero)
        .vector_narrow_split::<m![T1 = 1, Xg / 4], m![Xg % 4]>()
        .vector_fp_binary(FpBinaryOp::MulF(FpMulAlu::Mul0), DOWN_GAIN)
        .vector_widen_concat::<m![T1 = 1, Xg / 8], m![Xg % 8]>()
        .vector_final()
        .cast::<f8e4m3, m![Xg % 8 # 32]>()
        .commit_trim::<m![Xg % 8]>()
        .commit_view(q.view_mut());

    let q: DmTensor<f8e4m3, Chip, UpGateClusters, GroupsX, m![T1, Xd % 120]> = unsafe { q.reshape() };
    let gathered: DmTensor<f8e4m3, Chip, UpGateClusters, DownSlices, m![T1, Xd]> = device
        .main
        .begin(q.view())
        .fetch::<m![T1, Xd / 24 % 5], m![Xd % 24]>()
        .switch::<DownSlices, m![T1, Xd / 24 % 5, Xd / 120]>(SwitchConfig::Broadcast1 { slice1: 64, slice0: 4 })
        .collect::<m![T1, Xd / 24 % 5, Xd / 120], m![Xd % 24 # 32]>()
        .commit_trim::<m![Xd % 24]>()
        .commit();
    let gathered: DmTensor<f8e4m3, Chip, UpGateClusters, DownSlices, m![T1, Dt, Xp]> = unsafe { gathered.reshape() };
    device.sub
        .begin(gathered.view())
        .fetch::<m![T1, Dt, Xp / 32], m![Xp % 32]>()
        .collect::<m![T1, Dt, Xp / 32], m![Xp % 32]>()
        .to_trf()
}
...
fn down_scales_bf16(
    ...
    let s: DmTensor<f8e4m3, Chip, UpGateClusters, DownRowSlices, m![H / 2 % 15, L / 16 % 480]> = scale.to_dm(&mut device.tdma);
...
macro_rules! down_tile {
    ($device:ident, $w:ident, $x_trf:ident, $scale:ident, $out:ident, $start:literal, $len:literal) => {{
        ...
        let z: DmTensor<bf16, Chip, UpGateClusters, DownSlices, m![H / 2 % 15 = $len, Dt, Xp / 16]> = $device
            ...
            .contract_outer::<m![H / 2 % 15 = $len, Dt, T1], m![Xp], _, _, _>(&$x_trf)
            .contract_packet::<m![Xp / 16]>()
        ...
        let s_trf: TrfTensor<bf16, Chip, UpGateClusters, DownSlices, m![1], m![H / 2 % 15 = $len, Sd]> = $device
            .sub
...
pub(crate) fn feedforward(
    ...
    let (x_trf, residual_pieces) = normalize_quantize(device, residual, pre_ff_rms_weight);
    ...
    let up_z = block_sums_all!(device, up_weight_packed, x_trf);
    let up_s = scales_bf16(device, up_weight_scale);
    let up = apply_scales(device, &up_z, &up_s);
    let gate_z = block_sums_all!(device, gate_weight_packed, x_trf);
    let gate_s = scales_bf16(device, gate_weight_scale);
    let gate = apply_scales(device, &gate_z, &gate_s);

    let up = regroup(device, &up);
    let gate = regroup(device, &gate);
    let gate_factor = gate_factor_vrf(device, gate_global_scale);
    let x = geglu(device, &up, &gate, &gate_factor);
    let x_trf = quantize_gather(device, x);

    let down_s = down_scales_bf16(device, down_weight_scale);
    let mut partial: DmTensor<f32, Chip, UpGateClusters, DownSlices, m![H / 2 % 15, 1 # 8]> = DmTensor::new();
    // SDK 0.8.1 requires this DMA row factor to divide the 15-row local axis.
    // Three disjoint five-row tiles cover the same rows as the former 10+5 split.
    ...
    let partial: DmTensor<f32, Chip, UpGateClusters, m![H / 30, 1 # 2], m![H % 30 # 64]> = device
        .main
        .begin(partial.view())
    ...
    let partial: HbmTensor<f32, Chip, m![C2, Hg, Pw]> = partial.to_hbm(&mut device.tdma);
    ...
    let down_global_scale: DmTensor<f32, Chip, Cluster, m![1 # 32, Dummy8], m![1 # 8]> =
        down_global_scale.to_dm(&mut device.tdma);
    let down_global_scale: DmTensor<f32, Chip, Cluster, ReducingSlices, m![1 # 8]> =
        unsafe { down_global_scale.reshape() };
    let down_global_scale_vrf: VrfTensor<f32, Chip, Cluster, ReducingSlices, m![1 # 8]> = device
        .sub
    ...
    let up_global_scale_vrf: VrfTensor<f32, Chip, Cluster, ReducingSlices, m![1 # 8]> = device
        .sub
    ...
    let out: DmTensor<bf16, Chip, Cluster, ReducingSlices, m![H % 480]> = device
        .main
        .begin(partial.view())
...
pub(crate) fn normalize_add_gate_in_place(
    ...
    let mean_square: DmTensor<f32, Chip, Cluster, ReducingSlices, m![1 # 8]> = device
        .main
        .begin(x.view())
        .fetch::<m![H / 16 % 30], m![H % 16]>()
        .fetch_cast::<f32>()
        ...
        .commit_trim::<m![1 # 8]>()
        .commit();
    // Explicitly gather the eight H-group partials in each physical ring.
    // Every valid partial participates; only the cross-slice reduction implementation changes.
    let mean: DmTensor<f32, Chip, Cluster, m![1 # 32, Dummy8], m![1 # 8]> = device
        .main.begin(mean_square.view())
        .fetch::<m![1], m![1 # 8]>()
        .switch::<m![1 # 32, Dummy8], m![H / 480]>(SwitchConfig::Broadcast1 { slice1: 8, slice0: 1 })
    ...
    let rms: VrfTensor<f32, Chip, Cluster, m![1 # 32, Dummy8], m![1 # 8]> = device
        .main
        .begin(mean.view())
        .fetch::<m![1], m![1 # 8]>()
        .collect::<m![1], m![1 # 8]>()
    ...
    let rms_vrf: VrfTensor<f32, Chip, Cluster, ReducingSlices, m![1 # 8]> = unsafe { rms.reshape() };

    let weight_dm: DmTensor<bf16, Chip, Cluster, ReducingSlices, m![H % 480]> = rms_weight.to_dm(&mut device.tdma);
    let weight_vrf: VrfTensor<f32, Chip, Cluster, ReducingSlices, m![H % 480]> = device
        .sub
        .begin(weight_dm.view())
        .fetch::<m![H / 16 % 30], m![H % 16]>()
        .fetch_cast::<f32>()
        .collect::<m![H / 8 % 60], m![H % 8]>()
        .to_vrf();
    let residual_vrf: VrfTensor<f32, Chip, Cluster, ReducingSlices, m![H % 480]> = device
        .sub
        ...
        .collect::<m![H / 8 % 60], m![H % 8]>()
        .to_vrf();
    let scalar: DmTensor<bf16, Chip, Cluster, ReducingSlices, m![1 # 8]> = layer_scalar.to_dm(&mut device.tdma);
    let scalar_vrf: VrfTensor<f32, Chip, Cluster, ReducingSlices, m![1 # 8]> = device
        .sub
        .begin(scalar.view())
        .fetch::<m![1], m![1 # 8]>()
        .fetch_cast::<f32>()
