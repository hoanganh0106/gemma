...
pub(crate) fn normalize_native_input(
    ...
    let weight_vrf: VrfTensor<f32, Chip, QkvDualQueryCluster, Shards, m![H % 120]> = ctx
        ...
        .collect::<m![H / 8 % 15], m![H % 8]>()
        .to_vrf();
    // Quantize the weighted normalized input directly. This keeps input_rms_weight
    // in the live arithmetic path while removing the intermediate BF16 DM tensor
    // and the second Main fetch/logic pass.
