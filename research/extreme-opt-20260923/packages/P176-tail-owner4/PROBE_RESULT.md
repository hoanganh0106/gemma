...
## Compile probe — Tail owner `1#4`

- Source change: `Tail = m![1 # 8, H / 120]` -> `m![1 # 4, H / 120]`.
- Clean SDK 0.8.1 build reached MIR and rejected the ownership mapping: `DmTensor T240's slice extent 128 does not match the device config (slice = 256)`.
- This layout cannot allocate a device-wide DM tensor on the current 256 element slice configuration. No binary or Arena submission was produced.
