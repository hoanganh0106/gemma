...
- P190 tried to derive `sw` directly from the already materialized `scale` VRF, removing the second `scale_dm` fetch.
- Clean SDK 0.8.1 build rejected it: `begin` requires a `DmTensorView`, but `scale.view()` is a VRF `TensorView`; the following `vector_init` also has unsatisfied bounds.
