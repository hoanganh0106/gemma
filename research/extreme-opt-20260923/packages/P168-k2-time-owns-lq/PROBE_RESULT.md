...
- Attempted to move `Lq` from `contract_lane` into `contract_time`.
- MIR rejected the ownership: `OutTime (H % 120, Lq) must divide Time (H % 120, Qs / 32 % 8)`.
- The current packet32 time axis cannot expose `Lq` as an independent divisor; no schedule or Arena binary was produced.
