...
/// Contract one 60-row half, preserving the packet32 contraction used by P166.
pub(crate) fn contract_half(
    ...
    device.main
        ...
        .commit_view(z.view_mut().tile::<m![H % 120], 60, m![H % 120 = 60 #{!} 120]>(row_offset));
}
...
pub(crate) fn project_normalize_add(
    ...
    // Two producers fill disjoint row halves. A same-layout TDMA materialization then creates a
    // single canonical owner before the ring32 consumer, addressing P106's ownership failure.
    let mut z_split: Z = DmTensor::new();
    contract_half(device, &weight_lo, &x_trf, &mut z_split, 0);
    contract_half(device, &weight_hi, &x_trf, &mut z_split, 60);
    let z: Z = z_split.to_dm(&mut device.tdma);
