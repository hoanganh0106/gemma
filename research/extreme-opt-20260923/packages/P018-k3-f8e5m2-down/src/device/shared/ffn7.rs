...
fn quantize_gather(
...
) -> TrfTensor<f8e5m2, Chip, UpGateClusters, DownSlices, m![1], m![T2, Dt, Xp]> {
    let x: DmTensor<bf16, Chip, UpGateClusters, Groups, m![T2 = 1, Xg]> = unsafe { x.reshape() };
    let mut q: DmTensor<f8e5m2, Chip, UpGateClusters, Groups, m![T2, Xg]> = DmTensor::new();
    device.main
        .begin(x.view())
    ...
    let q: DmTensor<f8e5m2, Chip, UpGateClusters, GroupsX, m![T2, Xd % 120]> = unsafe { q.reshape() };
    let gathered: DmTensor<f8e5m2, Chip, UpGateClusters, DownSlices, m![T2, Xd]> = device
        .main
        .begin(q.view())
        ...
        .commit_trim::<m![Xd % 24]>()
        .commit();
    let gathered: DmTensor<f8e5m2, Chip, UpGateClusters, DownSlices, m![T2, Dt, Xp]> =
        unsafe { gathered.reshape() };
    device.sub
        .begin(gathered.view())
