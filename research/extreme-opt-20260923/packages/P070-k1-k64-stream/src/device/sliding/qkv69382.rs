...
pub(crate) fn project_key_streamed(
    ...
    project_kv_chunk_into(ctx, x_trf, first, 0, &mut out);
    for i in 0..3 {
        let offset = (i + 1) * 64;
        let chunk = load_kv_chunk(ctx, weight, offset);
        project_kv_chunk_into(ctx, x_trf, &chunk, offset, &mut out);
    }
    out
}
