...
use super::schema::{
    AudioSpeechRequest, ChatChoice, ChatCompletionChunk, ChatCompletionMessage,
    ChatCompletionRequest, ChatCompletionResponse, ChatMessage, ChunkChoice, ChunkDelta,
    CompletionChoice, CompletionChunk, CompletionRequest, CompletionResponse, ContentPart,
    ErrorResponse, MODEL_ID, MessageContent, ModelObject, ModelsList, OneOrMany, Usage,
};
...
fn error_param(
    ...
    json(
        status,
        &ErrorResponse::with_param(error_type, message, param),
    )
}

fn validate_n(n: Option<u32>) -> Result<(), HttpResponse> {
    match n {
        Some(n) if n != 1 => Err(error_param(
            400,
...
fn block_for_result(
    ...
    match rx.recv() {
        ...
        Ok(StreamEvent::Delta(_) | StreamEvent::ReasoningDelta(_)) | Err(_) => error(
            500,
            "internal_error",
            "generation worker closed unexpectedly",
        ),
    }
}
...
fn decode_data_url(url: &str) -> Result<Vec<u8>, String> {
    ...
    if !url.starts_with("data:") || !url[..comma].contains("base64") {
        return Err(
            "image_url.url must be a base64 data: URI -- remote URLs are not supported yet".into(),
        );
    }
    ...
    for message in messages {
        let content = match &message.content {
            ...
            Some(MessageContent::Parts(parts)) => {
                ...
                for part in parts {
                    match part {
                        ...
                        ContentPart::InputAudio { input_audio } => {
                            ...
                            let frames = audio::load_from_bytes(&bytes, &input_audio.format)
                                .map_err(|err| {
                                    error(
                                        400,
                                        "invalid_request_error",
                                        format!("could not decode audio: {err}"),
                                    )
                                })?;
                            audio = Some(frames);
                ...
                let json_parts: Vec<_> = image_marker
                    .into_iter()
                    .chain(text_parts)
                    .chain(audio_marker)
                    .collect();
...
fn sse_error(message: &str) -> Vec<u8> {
    let payload = serde_json::to_vec(&ErrorResponse::new("internal_error", message))
        .expect("error schema always serializes");
...
pub(crate) fn chat_completions(req: &HttpRequest, state: &AppState) -> HttpResponse {
    ...
    let (prompt_ids, soft_at) =
        match generate::splice_multimodal_tokens(chat_ids, image.as_ref(), audio.as_ref()) {
            Ok(value) => value,
    ...
    let sampling = SamplingConfig {
        ...
        top_k: body
            .top_k
            .unwrap_or(SamplingConfig::GEMMA_RECOMMENDED.top_k),
        top_p: body
            .top_p
            .unwrap_or(SamplingConfig::GEMMA_RECOMMENDED.top_p),
    };
            ...
            &ChatCompletionResponse {
                ...
                choices: vec![ChatChoice {
                    ...
                    message: ChatCompletionMessage {
                        role: "assistant",
                        reasoning_content: (!output.reasoning.is_empty())
                            .then_some(output.reasoning),
                        content: output.text,
                    },
...
pub(crate) fn completions(req: &HttpRequest, state: &AppState) -> HttpResponse {
    ...
    if prompts.len() != 1 {
        return error(
            400,
            "invalid_request_error",
            "only a single prompt string is supported",
        );
    }
