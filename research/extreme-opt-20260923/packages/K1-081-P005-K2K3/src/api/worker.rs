...
impl WorkerHandle {
    pub fn submit(&self, job: Job) {
        if let Err(mpsc::SendError(job)) = self.jobs.send(job) {
            let _ = job.events.send(StreamEvent::Error(
                "generation worker is not running".to_owned(),
            ));
        }
    }
...
fn run(tokenizer: Tokenizer, jobs: mpsc::Receiver<Job>, ready: mpsc::Sender<Result<(), String>>) {
    ...
    let setup = rt.block_on(async {
        let mut device =
            Device::new(crate::ops::embed_token.topology()).map_err(|err| err.to_string())?;
        let load_start = Instant::now();
        let model = load::load_model(&mut device)
            .await
