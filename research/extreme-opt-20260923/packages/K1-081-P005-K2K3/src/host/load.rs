...
> {
    Ok((
        load_f4(
            device,
            tensors,
            &format!("{prefix}.{name}.weight_packed"),
            shape,
        )
        .await?,
        ...
        load_global_scale(
            device,
            tensors,
            &format!("{prefix}.{name}.weight_global_scale"),
        )
        .await?,
    ))
}
...
async fn load_mlp(
    ...
    let (up_weight_packed, up_weight_scale, up_global_scale) =
        load_mlp_projection::<m![L, H], m![L, H / 16]>(
            device,
            tensors,
            prefix,
            "up_proj",
    ...
    let (down_weight_packed, down_weight_scale, down_global_scale) =
        load_mlp_projection::<m![H, L], m![H, L / 16]>(
            device,
            tensors,
            prefix,
            "down_proj",
...
async fn load_sliding(
    ...
    let (pre_feedforward_norm, post_feedforward_norm, input_norm, mlp) =
        load_common(device, tensors, &prefix).await?;
    Ok(SlidingLayer {
        input_norm,
        q_weight: load_f8(
            device,
        ...
        .await?,
        q_weight_scale: load_bf16_column(
            device,
            tensors,
            &format!("{prefix}.self_attn.q_proj.weight_scale"),
        )
        .await?,
        k_weight_scale: load_bf16_column(
            device,
            tensors,
            &format!("{prefix}.self_attn.k_proj.weight_scale"),
        )
        .await?,
        v_weight_scale: load_bf16_column(
            device,
            tensors,
            &format!("{prefix}.self_attn.v_proj.weight_scale"),
        )
        .await?,
        q_norm: load_bf16(
            device,
        ...
        .await?,
        o_weight_scale: load_bf16_column(
            device,
            tensors,
            &format!("{prefix}.self_attn.o_proj.weight_scale"),
        )
        .await?,
...
async fn load_vision(
    ...
    let pos_embedding_host: HostTensor<bf16, m![Pv, Dummy2, Mv]> =
        HostTensor::from_safetensors(&pos_embedding_view)
            .map_err(|error| format!("pos_embedding: {error}"))?;

    Ok(VisionWeights {
        patch_ln1_weight: load_bf16(device, tensors, "model.vision_embedder.patch_ln1.weight")
            .await?,
        patch_ln1_bias: load_bf16(device, tensors, "model.vision_embedder.patch_ln1.bias").await?,
        patch_dense_weight: load_bf16(device, tensors, "model.vision_embedder.patch_dense.weight")
            .await?,
        patch_dense_bias: load_bf16(device, tensors, "model.vision_embedder.patch_dense.bias")
            .await?,
        patch_ln2_weight: load_bf16(device, tensors, "model.vision_embedder.patch_ln2.weight")
            .await?,
        patch_ln2_bias: load_bf16(device, tensors, "model.vision_embedder.patch_ln2.bias").await?,
        pos_norm_weight: load_bf16(device, tensors, "model.vision_embedder.pos_norm.weight")
            .await?,
        pos_norm_bias: load_bf16(device, tensors, "model.vision_embedder.pos_norm.bias").await?,
        embedding_projection_weight: load_bf16(
            device,
            tensors,
...
pub async fn load_model(device: &mut Device) -> Result<Model, Box<dyn std::error::Error>> {
    ...
    let embedding_table =
        load_bf16(device, &tensors, "model.language_model.embed_tokens.weight").await?;
