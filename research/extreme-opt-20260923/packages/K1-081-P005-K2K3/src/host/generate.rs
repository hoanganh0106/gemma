...
pub fn splice_multimodal_tokens(
    ...
    if image.is_some() && image_markers != 1 {
        return Err(format!(
            "expected one image placeholder, found {image_markers}"
        ));
    }
    if audio.is_some() && audio_markers != 1 {
        return Err(format!(
            "expected one audio placeholder, found {audio_markers}"
        ));
    }
...
pub async fn generate(
        ...
        match soft_token {
            Some(SoftToken::Image(i)) => {
                let patches = req
                    .image
                    .as_ref()
                    .expect("soft_at only set when an image was loaded");
                let embedding = runtime::encode_vision_patch(
                    device,
                    model,
                    &patches.pixels[*i],
                    patches.positions[*i],
            ...
            Some(SoftToken::Audio(i)) => {
                let audio = req
                    .audio
                    .as_ref()
                    .expect("soft_at only set when audio was loaded");
                let embedding =
                    runtime::encode_audio_frame(device, model, &audio.frames[*i]).await?;
                decode
                    .run_with_embedding(device, workspace, embedding, model, pos, compute_logits)
                    .await?;
            }
            None => {
                decode
                    .run(device, workspace, token, model, pos, compute_logits)
                    .await?;
            }
        }
    }
    ...
    while !EOS_TOKENS.contains(&next) && generated_count < req.max_new_tokens {
        ...
        decode
            .run(device, workspace, next, model, pos, true)
            .await?;
