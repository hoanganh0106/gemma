...
pub fn load_from_bytes(
    bytes: &[u8],
    format: &str,
) -> Result<AudioFrames, Box<dyn std::error::Error>> {
...
fn decode_wav(bytes: &[u8]) -> Result<(u32, Vec<f32>), Box<dyn std::error::Error>> {
    ...
    let bytes_per_sample = match (audio_format, bits) {
        ...
        _ => {
            return Err(format!(
                "unsupported WAV sample format: format={audio_format}, bits={bits}"
            )
            .into());
        }
    };
...
fn resample_mono(
    ...
    if samples.len()
        > MAX_SAMPLES.saturating_mul(from as usize) / SAMPLE_RATE as usize + from as usize
    {
