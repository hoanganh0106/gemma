...
fn patch_partial(
    ...
    let x: DmTensorView<'_, bf16, Chip, Cluster, EmbeddingRows, m![Rv]> =
    ...
    let weight_half = weight
        .view()
        .tile::<m![Rv], 768, m![Mv, Rv = 768 # 6912]>(offset);
...
pub(crate) fn patch_projection(
    ...
    let bias: DmTensor<bf16, Chip, Cluster, EmbeddingRows, m![Mv % 120]> =
        bias.to_dm(&mut device.tdma);
    ...
    device
        ...
        .switch::<Slice, m![Mv / 8 % 15, Mv / 120]>(SwitchConfig::Broadcast1 {
            slice1: 32,
            slice0: 8,
        })
...
fn project_partial(
    ...
    let x: DmTensorView<'_, bf16, Chip, Cluster, HiddenRows, m![Ov]> =
        unsafe { x.view().reshape() };
    ...
    let weight_half = weight
        .view()
        .tile::<m![Ov], 768, m![H, Ov = 768 # 3840]>(offset);
...
pub(crate) fn project_to_text_hidden(
    ...
    device
        ...
        .switch::<Slice, m![H / 8 % 15, H / 120]>(SwitchConfig::Broadcast1 {
            slice1: 32,
            slice0: 8,
        })
