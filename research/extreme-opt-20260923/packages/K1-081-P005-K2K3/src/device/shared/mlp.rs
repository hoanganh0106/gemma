...
pub(crate) fn project_up_and_gate(
    ...
    for i in 0..PASSES {
        let up_weight_scale_vrf: VrfTensor<f32, Chip, Cluster, UpGateRows, m![L % 60 = 4, H / 16]> =
            device
                .sub
                .begin(
                    up_weight_scale
...
pub(crate) fn geglu(
    ...
    let up: DmTensor<bf16, Chip, Cluster, UpGateRowsPaired, m![L % 120]> = device
        ...
        .switch::<UpGateRowsPaired, m![L / 4 % 15, L / 60 % 2]>(SwitchConfig::Broadcast1 {
            slice1: 2,
            slice0: 1,
        })
...
pub(crate) fn project_down(
    ...
    let x: DmTensor<bf16, Chip, Cluster, DownRowsByColumns, m![L % 1920]> =
    ...
    for i in 0..PASSES {
        ...
        for j in 0..HALVES_PER_PASS {
            let down_weight_packed: DmTensor<f4e2m1, Chip, Cluster, DownRows, m![H % 120 = 2, L]> =
                down_weight_packed
                    .view()
                    .tile::<m![H % 120], 2, m![H / 120, H % 120 = 2 # 120, L]>(4 * i + 2 * j)
                    .to_dm(&mut device.tdma);
            let down_weight_packed: DmTensor<f8e4m3, Chip, Cluster, DownRows, m![H % 120 = 2, L]> =
                device
                    .main
            ...
            let down_weight_scale_vrf: VrfTensor<
                f32,
                Chip,
                Cluster,
                DownRows,
            ...
            let down_weight_tile: DmTensor<bf16, Chip, Cluster, DownRows, m![H % 120 = 2, L]> =
                device
                    .main
                    .begin(down_weight_packed.view())
        ...
        device
            ...
            .commit_view(
                down.view_mut()
                    .tile::<m![H % 120], 4, m![H % 120 = 4 #{!} 120]>(4 * i),
            );
    }
