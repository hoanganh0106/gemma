...
fn query_key_scores(
    ...
    let q: DmTensor<bf16, Chip, Cluster, KvRowsAcrossSlices, m![Gf, Df]> =
        q.to_dm(&mut device.tdma);
    let k: DmTensor<bf16, Chip, Cluster, KvRowsAcrossSlices, m![Tf % 8, Df]> =
        k.to_dm(&mut device.tdma);
    let k_trf: TrfTensor<bf16, Chip, Cluster, KvRowsAcrossSlices, m![Tf % 8], m![Df]> = device
        .sub
        .begin(k.view())
    ...
    let qk: DmTensor<bf16, Chip, Cluster, KvRowsAcrossSlices, m![Gf, Tf % 8]> = device
        .main
        .begin(q.view())
    ...
    device
        ...
        .switch::<Slice, m![Gf, Tf / 8]>(SwitchConfig::Broadcast1 {
            slice1: 64,
            slice0: 4,
        })
...
fn masked_exp(
    ...
    let exp: DmTensor<f32, Chip, Cluster, Slice, m![Gf, Tf]> = device
        .main
        .begin(qk.view())
        .fetch::<m![Gf, Tf / 16], m![Tf % 16]>()
        .fetch_cast::<f32>()
    ...
    let mask: DmTensor<f32, Chip, Cluster, Slice, m![Tf]> = mask.to_dm(&mut device.tdma);
    let mask_vrf: VrfTensor<f32, Chip, Cluster, Slice, m![Tf]> = device
        .sub
        .begin(mask.view())
        .fetch::<m![Tf / 8], m![Tf % 8]>()
...
fn attention_weighted_values(
    ...
    let attention_weights: DmTensor<bf16, Chip, Cluster, Slice, m![Gf, Tf]> = device
        .main
        .begin(masked_exp.view())
        .fetch::<m![Gf, Tf / 8], m![Tf % 8]>()
        .collect::<m![Gf, Tf / 8], m![Tf % 8]>()
    ...
    let v: DmTensor<bf16, Chip, Cluster, QueryHeadAndRows, m![Tf % 32, Df]> =
        v.to_dm(&mut device.tdma);
    let v: DmTensor<bf16, Chip, Cluster, QueryHeadAndRows, m![Df, Tf % 32]> = device
        .main
        .begin(v.view())
        ...
        .commit_trim::<m![Tf % 4]>()
        .commit();
    let v_trf: TrfTensor<bf16, Chip, Cluster, QueryHeadAndRows, m![Df % 8], m![Df / 8, Tf % 32]> =
        device
            .sub
            .begin(v.view())
    ...
    let weighted_sum: DmTensor<bf16, Chip, Cluster, QueryHeadAndRows, m![Df]> = device
        .main
...
pub(crate) fn attend_first_page(
    ...
    let qk = query_key_scores(device, q, k);
    let max = row_max(device, &qk);
    let max_vrf = per_head_to_vrf(device, &max);

    let exp = masked_exp(device, &qk, &max_vrf, mask);
    let sum = row_sum(device, &exp);
    let result = attention_weighted_values(device, &exp, v);

    max.view()
        .to_hbm_view(&mut device.tdma, running_max.view_mut());
    sum.view()
        .to_hbm_view(&mut device.tdma, running_sum.view_mut());
    result
        .view()
        .to_hbm_view(&mut device.tdma, out_hbm.view_mut());
}
...
pub(crate) fn attend_next_page(
    ...
    let old_max: DmTensor<f32, Chip, Cluster, Slice, m![Gf]> = running_max.to_dm(&mut device.tdma);
    let old_max_vrf = per_head_to_vrf(device, &old_max);

    let max: DmTensor<f32, Chip, Cluster, Slice, m![Gf]> = device
        .main
        .begin(max.view())
        .fetch::<m![Gf / 8], m![Gf % 8]>()
        .collect::<m![Gf / 8], m![Gf % 8]>()
    ...
    let rescale: DmTensor<f32, Chip, Cluster, Slice, m![Gf]> = device
        .main
        .begin(max.view())
        .fetch::<m![Gf / 8], m![Gf % 8]>()
        .collect::<m![Gf / 8], m![Gf % 8]>()
        ...
        .commit_trim::<m![Gf % 8]>()
        .commit();
    let rescale_vrf = per_head_to_vrf(device, &rescale);
    let max_vrf = per_head_to_vrf(device, &max);

    let exp = masked_exp(device, &qk, &max_vrf, mask);
    let expsum = row_sum(device, &exp);
    let expsum_vrf = per_head_to_vrf(device, &expsum);

    let sum: DmTensor<f32, Chip, Cluster, Slice, m![Gf]> = running_sum.to_dm(&mut device.tdma);
    let sum: DmTensor<f32, Chip, Cluster, Slice, m![Gf]> = device
        .main
        .begin(sum.view())
        .fetch::<m![Gf / 8], m![Gf % 8]>()
        .collect::<m![Gf / 8], m![Gf % 8]>()
    ...
    let result = attention_weighted_values(device, &exp, v);
    let result_vrf: VrfTensor<f32, Chip, Cluster, PerQueryHead, m![Df]> = device
        .sub
        .begin(result.view())
        .fetch::<m![Df / 16], m![Df % 16]>()
        .fetch_cast::<f32>()
    ...
    let out: DmTensor<bf16, Chip, Cluster, Slice, m![Gf, Df]> = out_hbm.to_dm(&mut device.tdma);
    let out: DmTensor<bf16, Chip, Cluster, Slice, m![Gf, Df]> = device
        .main
        .begin(out.view())
        .fetch::<m![Gf, Df / 16], m![Df % 16]>()
        .fetch_cast::<f32>()
    ...
    let out: DmTensor<bf16, Chip, Cluster, PerQueryHead, m![Df]> = out_hbm.to_dm(&mut device.tdma);
    let out: DmTensor<bf16, Chip, Cluster, PerQueryHead, m![Df]> = device
        .main
        .begin(out.view())
        .fetch::<m![Df / 16], m![Df % 16]>()
        .fetch_cast::<f32>()
...
pub(crate) fn divide_by_softmax_sum<Slice: M>(
    ...
    let sum: DmTensor<f32, Chip, Cluster, Slice, m![Gf]> = running_sum.to_dm(&mut device.tdma);
    let sum_vrf = per_head_to_vrf(device, &sum);
