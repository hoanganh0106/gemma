...
pub(crate) fn project_query(
    ...
    let weight_f8: DmTensor<f8e4m3, Chip, Cluster, QueryRows, m![Qf % 32, H]> =
    ...
    let weight_scale: DmTensor<bf16, Chip, Cluster, QueryRows, m![Qf % 32]> =
    ...
    let result: DmTensor<bf16, Chip, Cluster, Slice, m![Qf]> = device
        ...
        .begin(result.view())
        .fetch::<m![1], m![Qf % 32]>()
        .switch::<Slice, m![Qf / 32]>(SwitchConfig::Broadcast1 {
            slice1: 256,
            slice0: 1,
        })
...
pub(crate) fn project_key(
    ...
    let weight_f8: DmTensor<f8e4m3, Chip, Cluster, KvRows, m![Pf % 8, H]> =
        weight.to_dm(&mut device.tdma);
    ...
    let weight_scale: DmTensor<bf16, Chip, Cluster, KvRows, m![Pf % 8]> =
        weight_scale.to_dm(&mut device.tdma);
    ...
    let result: DmTensor<bf16, Chip, Cluster, Slice, m![Pf]> = device
        ...
        .begin(result.view())
        .fetch::<m![1], m![Pf % 8 # 16]>()
        .switch::<Slice, m![Pf / 8]>(SwitchConfig::Broadcast1 {
            slice1: 64,
            slice0: 4,
        })
...
fn apply_output_channel_scale(
    ...
    let weight_scale: DmTensor<bf16, Chip, Cluster, HiddenRows, m![H % 120]> =
        weight_scale.to_dm(&mut device.tdma);
...
fn output_partial(
    ...
    let x: DmTensorView<'_, bf16, Chip, Cluster, HiddenRows, m![Qf]> =
