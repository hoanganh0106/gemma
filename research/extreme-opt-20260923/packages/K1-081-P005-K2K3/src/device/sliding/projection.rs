...
pub(crate) fn project_query(
    ...
    let output: DmTensor<bf16, Chip, Cluster, Slice, m![Qs]> = device
        ...
        .begin(scaled.view())
        .fetch::<m![1], m![Qs % 16]>()
        .switch::<Slice, m![Qs / 16]>(SwitchConfig::Broadcast1 {
            slice1: 256,
            slice0: 1,
        })
...
fn project_one_kv_matrix(
    ...
    let weight_scale: DmTensor<bf16, Chip, Cluster, KvRows, m![Ps % 8]> =
        weight_scale.to_dm(&mut device.tdma);
    ...
    device
        ...
        .begin(scaled.view())
        .fetch::<m![1], m![Ps % 8 # 16]>()
        .switch::<Slice, m![Ps / 8]>(SwitchConfig::Broadcast1 {
            slice1: 256,
            slice0: 1,
        })
...
fn output_partial(
    ...
    let x: DmTensorView<'_, bf16, Chip, Cluster, HiddenRows, m![Qs]> =
