...
## Coupled packet16 probe

- Changed fetch/collect and `contract_outer` together to use `Qs % 16`, addressing P113's mapping-coupling error.
- MIR progressed, but the SDK rejected the collect layout: `Collect output packet must be exactly 32 bytes (one flit): 16 elements = 16 bytes`.
- The lower packet bound is therefore 32 bytes; no schedule, binary, or Arena submission was produced.
