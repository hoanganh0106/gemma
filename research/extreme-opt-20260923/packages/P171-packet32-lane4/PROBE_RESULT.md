...
## Compile probe — packet32 lane4

- Source change: `.contract_lane(..., m![1 # 8])` -> `m![1 # 4]`.
- Clean SDK 0.8.1 build reached MIR validation and rejected the mapping:
  `contract_lane: OutPacket::SIZE must be 8, got 4`.
- No binary was produced and no Arena submission was made. The packet32 contract requires the 8 element output packet, so this lane4 branch is not a viable ownership family.
