...
fn project_one_kv_matrix(
    ...
    let weight: DmTensor<f8e4m3, Chip, KvCluster, KvRows, m![Ps % 8, H]> =
        weight.to_dm(&mut device.tdma);
    // Relabel fully initialized row partitions, without changing their wire order.
    let weight: DmTensor<f8e4m3, Chip, QkvDualQueryCluster, NativeSlices, m![Ps % 8, H]> =
        unsafe { weight.reshape() };
    let contraction: DmTensor<bf16, Chip, QkvDualQueryCluster, NativeSlices, m![Ps % 8]> = device.main
        .begin(weight.view())
        ...
        .contract_lane::<m![Ps % 8, Dummy2], m![1 # 8]>(LaneMode::Sequential)
        .vector_init()
        .vector_intra_slice_tag(TagMode::Zero)
        .vector_narrow_trim::<m![1 # 4]>()
        .vector_fp_binary(FpBinaryOp::MulF(FpMulAlu::Mul0), x_scale)
        .vector_intra_slice_reduce::<Dummy2, m![Ps % 8], m![1 # 4]>(IntraSliceReduceOpF32::Add)
        .vector_widen_pad::<m![1 # 8]>()
        .vector_final()
        .cast::<bf16, m![1 # 16]>()
        .transpose::<m![1], m![Ps % 8 # 16]>()
        .commit_trim::<m![Ps % 8]>()
        .commit();
    let scaled = contraction;
