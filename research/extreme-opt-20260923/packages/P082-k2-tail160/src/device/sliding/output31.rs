...
pub(crate) type Tail = m![1 # 8, H / 160];
pub(crate) type TailDm<D> = DmTensor<D, Chip, Vc, Tail, m![H % 160]>;
pub(crate) type TailVrf = VrfTensor<f32, Chip, Vc, Tail, m![H % 160]>;

pub(crate) type XTrf = TrfTensor<f8e4m3, Chip, OutputClusters, SlidingOutputColumns, m![Lq], m![Qs % 256]>;
pub(crate) type Z = DmTensor<bf16, Chip, OutputClusters, Rows, m![H % 160]>;

pub(crate) type WeightTile = DmTensor<f8e4m3, Chip, OutputClusters, SlidingOutputColumns, m![H % 160, Qs % 256]>;
...
pub(crate) fn contract_tile(device: &mut Device, weight: &WeightTile, x_trf: &XTrf, z: &mut Z) {
    device.main
        .begin(weight.view())
        .fetch::<m![H % 160, Qs / 32 % 8], m![Qs % 32]>()
        .collect::<m![H % 160, Qs / 32 % 8], m![Qs % 32]>()
        .contract_outer::<m![H % 160, Qs / 64 % 4], m![Qs % 64], _, _, _>(x_trf)
        .contract_packet::<m![1]>()
        .contract_time::<m![H % 160]>()
        .contract_lane::<m![H % 160, Lq], m![1 # 8]>(LaneMode::Sequential)
        .vector_init()
        .vector_intra_slice_tag(TagMode::Zero)
        .vector_narrow_trim::<m![1 # 4]>()
        .vector_intra_slice_reduce::<Lq, m![H % 160], m![1 # 4]>(IntraSliceReduceOpF32::Add)
        .vector_widen_pad::<m![1 # 8]>()
        .vector_inter_slice_reduce::<Rows, m![H % 160]>(InterSliceReduceOpF32::Add)
        .vector_final()
        .cast::<bf16, m![1 # 16]>()
        .transpose::<m![H % 160 / 4], m![H % 160 % 4 # 16]>()
        .commit_trim::<m![H % 160 % 4]>()
...
pub(crate) fn project_normalize_add(
    ...
    let x: DmTensor<bf16, Chip, OutputClusters, m![H / 160 % 16, Ns, Gs], m![Ds]> = x.to_dm(&mut device.tdma);
    ...
    let mean: DmTensor<f32, Chip, Vc, m![1 # 8, Vr], m![1 # 8]> = device
        ...
        .fetch::<m![1], m![1 # 8]>()
        .switch::<m![1 # 8, Vr], m![H / 160]>(SwitchConfig::Broadcast1 { slice1: 32, slice0: 1 })
        .collect::<m![H / 160], m![1 # 8]>()
