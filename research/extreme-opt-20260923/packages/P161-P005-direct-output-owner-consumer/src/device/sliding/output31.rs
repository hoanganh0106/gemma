...
pub(crate) fn project_normalize_add(
    ...
    // Consumer-owner microprobe: keep projection ownership and test direct use.
    let z_tail = z;
    ...
    let partial = device
        ...
        .vector_intra_slice_tag(TagMode::Zero)
        .vector_narrow_split::<m![H / 4 % 30], m![H % 4]>()
        .vector_fp_binary(FpBinaryOp::MulF(FpMulAlu::Mul0), &unsafe { scale.reshape::<m![1], m![H / 1920], m![H / 120 % 16, 1 # 16], m![H % 120]>() })
        .vector_stash()
    ...
    let mean = device
        ...
        .fetch::<m![1], m![1 # 8]>()
        .switch::<m![H / 240 % 8, 32], m![H / 960]>(SwitchConfig::Broadcast1 { slice1: 32, slice0: 1 })
        .collect::<m![H / 240], m![1 # 8]>()
        .vector_init().vector_intra_slice_tag(TagMode::Zero)
        .vector_narrow_trim::<m![1 # 4]>()
        .vector_intra_slice_reduce::<H, m![1], m![1 # 4]>(IntraSliceReduceOpF32::Add)
        .vector_widen_pad::<m![1 # 8]>()
        .vector_final().commit_trim::<m![1 # 8]>().commit::<m![1 # 8]>();
    ...
    let inv_rms = device
        ...
        .vector_widen_pad::<m![1 # 8]>()
        .vector_final()
        .to_vrf::<m![1 # 8]>(&mut device.sub);
    // Same physical slice grouping as the former DM reshape followed by Sub fetch.
    let inv_rms_vrf = unsafe {
        inv_rms.reshape::<m![1], m![H / 1920], m![H / 120 % 16, 1 # 16], m![1 # 8]>()
    };
    ...
    let out = device
        ...
        .vector_intra_slice_tag(TagMode::Zero)
        .vector_narrow_split::<m![H / 4 % 30], m![H % 4]>()
        .vector_fp_binary(FpBinaryOp::MulF(FpMulAlu::Mul0), &unsafe { sw.reshape::<m![1], m![H / 1920], m![H / 120 % 16, 1 # 16], m![H % 120]>() })
        .vector_fp_binary(FpBinaryOp::MulF(FpMulAlu::Mul1), &inv_rms_vrf)
        .vector_widen_concat::<m![H / 8 % 15], m![H % 8]>()
        .vector_clip(ClipBinaryOpF32::Add, &unsafe { residual.reshape::<m![1], m![H / 1920], m![H / 120 % 16, 1 # 16], m![H % 120]>() })
        .vector_final()
        .cast::<bf16, m![H % 8 # 16]>()
        .commit_trim::<m![H % 8]>()
        .commit::<m![H % 8]>();
