...
pub fn sliding_project_qkv(
    ...
    let x = sliding::qkv_head_local::normalize_native_input(device, x, input_rms_weight);
    let q = sliding::qkv_head_local::project_query(device, &x, q_weight, q_weight_scale);
    let (k, v) = sliding::qkv_head_local::project_key_value(device, &x, k_weight, v_weight, k_weight_scale, v_weight_scale);
    let q = sliding::qkv_head_local::normalize_weighted(device, &q, q_rms_weight);
    let k = sliding::qkv_head_local::normalize_key_project_scaled(device, &k, k_weight_scale, k_rms_weight);
