...
axes![Rep8 = 8, Q4 = 4, Rep32 = 32, Ring8 = 8, T2 = 2, Xh = 3840, Rep4 = 4, Ring64 = 64, Xg = 120, Xd = 7680, Ut = 60, Dt = 120, Xp = 64, C2 = 2, Lead4 = 4, Hg = 128, Pw = 64, Sb = 240, Sd = 480, UG2 = 2];
...
macro_rules! block_sums_pair {
    ($device:ident, $u:ident, $g:ident, $x_trf:ident) => {{
        let mut packed: DmTensor<f4e2m1, Chip, UpGateClusters, RowSlices, m![UG2, L % 30, H]> = DmTensor::new();
        ...
        let packed: DmTensor<f4e2m1, Chip, UpGateClusters, Gathered, m![UG2, L % 30, Ut, Xp]> =
            unsafe { packed.reshape() };
        let z: DmTensor<bf16, Chip, UpGateClusters, Gathered, m![UG2, L % 30, Ut, Xp / 16]> = $device
            .main
            .begin(packed.view())
            .fetch::<m![UG2, L % 30, Ut], m![Xp]>()
            .fetch_table_lookup::<f8e4m3>()
            .collect::<m![UG2, L % 30, Ut, Xp / 32], m![Xp % 32]>()
            .contract_outer::<m![UG2, L % 30, Ut, T2], m![Xp], _, _, _>(&$x_trf)
            .contract_packet::<m![Xp / 16]>()
            .contract_time::<m![UG2, L % 30, Ut]>()
            .contract_lane::<m![UG2, L % 30, Ut], m![Xp / 16 # 8]>(LaneMode::Sequential)
            .cast::<bf16, m![Xp / 16 # 16]>()
            .commit_trim::<m![Xp / 16]>()
            .commit();
        let z: DmTensor<bf16, Chip, UpGateClusters, Gathered, m![UG2, L % 30, Sb]> = unsafe { z.reshape() };
        z
    }};
}
...
fn apply_scales(
    device: &mut Device,
    z: DmTensorView<bf16, Chip, UpGateClusters, Gathered, m![UG2 = 1 # 2, L % 30, Sb]>,
...
fn geglu(
    ...
    let gelu: DmTensor<f32, Chip, UpGateClusters, Groups, m![L % 120]> = device
        .sub
        .begin(gate.view())
...
fn quantize_gather(
    ...
    let q1_vrf: VrfTensor<f32, Chip, UpGateClusters, Groups, m![T2 = 1, Xg]> = device
        .sub
    ...
    let gathered: DmTensor<f8e4m3, Chip, UpGateClusters, DownSlices, m![T2, Xd]> = device
        .main
        .begin(q.view())
...
pub(crate) fn feedforward(
    ...
    let pair_z = block_sums_pair!(device, up_weight_packed, gate_weight_packed, x_trf);
    let up_z = pair_z.view().tile::<m![UG2], 1, m![UG2 = 1 # 2, L % 30, Sb]>(0);
    let gate_z = pair_z.view().tile::<m![UG2], 1, m![UG2 = 1 # 2, L % 30, Sb]>(1);
...
pub(crate) fn normalize_add_gate_in_place(
    ...
    let mean_square: DmTensor<f32, Chip, Cluster, ReducingSlices, m![1 # 8]> = device
        ...
        .commit_trim::<m![1 # 8]>()
        .commit();
    let rms: DmTensor<f32, Chip, Cluster, m![1 # 32, Dummy8], m![1 # 8]> = device
        .main
        .begin(mean_square.view())
        .fetch::<m![1], m![1 # 8]>()
        .collect::<m![1], m![1 # 8]>()
    ...
    let weight_vrf: VrfTensor<f32, Chip, Cluster, ReducingSlices, m![H % 480]> = device
        ...
        .collect::<m![H / 8 % 60], m![H % 8]>()
        .to_vrf();
    let rms_vrf: VrfTensor<f32, Chip, Cluster, ReducingSlices, m![1 # 8]> = device
        .sub
        .begin(rms.view())
        .fetch::<m![1], m![1 # 8]>()
        .collect::<m![1], m![1 # 8]>()
