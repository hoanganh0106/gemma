...
macro_rules! down_tile {
    ($device:ident, $w:ident, $x_trf:ident, $scale:ident, $out:ident, $start:literal, $len:literal) => {{
        let packed: DmTensorView<f4e2m1, Chip, UpGateClusters, DownSlices, m![H / 2 % 15 = $len, Dt, Xp]> = unsafe { $w
            .view()
...
pub(crate) fn feedforward(
    ...
    down_tile!(device, down_weight_packed, x_trf, down_s, partial, 0, 10);
