...
for name in "$@"; do
  ...
  (cd work && flock /tmp/furiosa-extreme-20260923.lock cargo furiosa-opt compile ops::decoder_feedforward --exact --dump-schedule "../$name/schedule.json") > "$name/compile.log" 2>&1
  code=$?
  echo "$name exit=$code"
