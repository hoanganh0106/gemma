...
pub(crate) type Tail = m![1 # 16, H / 960];
...
pub(crate) fn project_normalize_add(
    ...
    let mean: DmTensor<f32, Chip, Vc, m![1 # 4, Vr], m![1 # 8]> = device
        .main.begin(partial.view())
        .fetch::<m![1], m![1 # 8]>()
        .switch::<m![1 # 4, Vr], m![H / 960]>(SwitchConfig::Broadcast1 { slice1: 4, slice0: 1 })
    ...
    let inv_rms: VrfTensor<f32, Chip, Vc, m![1 # 4, Vr], m![1 # 8]> = device
        .main
        .begin(mean.view())
        .fetch::<m![1], m![1 # 8]>()
        .collect::<m![1], m![1 # 8]>()
