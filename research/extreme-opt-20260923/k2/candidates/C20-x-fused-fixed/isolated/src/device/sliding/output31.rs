...
type InputSlices = m![1 # 16, Qs / 256];
...
pub(crate) fn quantise_x(
    device: &mut Device,
    x: &DmTensor<bf16, Chip, OutputClusters, InputSlices, m![Qs % 256]>,
) -> XTrf {
    ...
    device.main
        ...
        .fetch::<m![1], m![Qs % 256]>()
        .fetch_cast::<f32>()
        .switch::<SlidingOutputColumns, m![1]>(SwitchConfig::Broadcast1 { slice1: 16, slice0: 16 })
...
pub(crate) fn project_normalize_add(
    ...
    // HBM only initializes the first 16 slices with the 16 x blocks on each cluster.
    ...
    let mean: DmTensor<f32, Chip, Vc, m![1 # 16, Vr], m![1 # 8]> = device
        ...
        .fetch::<m![1], m![1 # 8]>()
        .switch::<m![1 # 16, Vr], m![H / 240]>(SwitchConfig::CustomBroadcast { ring_size: 16 })
