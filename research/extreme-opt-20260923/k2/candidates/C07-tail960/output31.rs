...
pub(crate) type Tail = m![1 # 64, H / 960];
pub(crate) type TailDm<D> = DmTensor<D, Chip, Vc, Tail, m![H % 960]>;
pub(crate) type TailVrf = VrfTensor<f32, Chip, Vc, Tail, m![H % 960]>;
...
pub(crate) fn project_normalize_add(
    ...
    let inv_rms: VrfTensor<f32, Chip, Vc, m![1 # 64, Vr], m![1 # 8]> = device
        .main
        .begin(partial.view())
        .fetch::<m![1], m![1 # 8]>()
        .collect::<m![1], m![1 # 8]>()
        .vector_init()
        .vector_inter_slice_reduce::<m![1 # 64, Vr], m![1]>(InterSliceReduceOpF32::Add)
        .vector_intra_slice_tag(TagMode::Zero)
        .vector_narrow_trim::<m![1 # 4]>()
