...
pub(crate) fn project_normalize_add(
    ...
    // Gather the 32 group partials explicitly within each 32-slice ring.
    // Each active tail slice receives all 32 H-group partials as time values.
    let mean: DmTensor<f32, Chip, Vc, m![1 # 8, Vr], m![1 # 8]> = device
        .main.begin(partial.view())
        .fetch::<m![1], m![1 # 8]>()
        .switch::<m![1 # 8, Vr], m![H / 120]>(SwitchConfig::CustomBroadcast { ring_size: 32 })
