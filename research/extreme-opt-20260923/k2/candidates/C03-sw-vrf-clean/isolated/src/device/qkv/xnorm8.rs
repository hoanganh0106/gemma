...
pub(crate) fn normalize_everywhere_f8<Cluster: M>(
    ...
    let mut xpool: DmTensor<bf16, Chip, Cluster, m![Rep16, H / 240], m![Slot, H % 240]> = DmTensor::new();
    {
        ...
        w.to_dm_view(&mut device.tdma, xpool.view_mut().tile::<m![Slot], 1, m![Slot = 1 #{!} 2, H % 240]>(1));
        let xv = unsafe { x.view().reshape::<Chip, m![Slot = 1, H]>() };
        xv.to_dm_view(&mut device.tdma, xpool.view_mut().tile::<m![Slot], 1, m![Slot = 1 #{!} 2, H % 240]>(0));
    }
    let mean_square: DmTensor<f32, Chip, Cluster, m![Rep16, H / 240], m![1 # 8]> = device
        .main
        ...
        .commit_trim::<m![1 # 8]>()
        .commit();
    let reduced: DmTensor<f32, Chip, Cluster, m![Rep16, Ring16], m![1 # 8]> = device
        .main
        ...
        .commit_trim::<m![1 # 8]>()
        .commit();
    let rms: DmTensor<f32, Chip, Cluster, m![Rep16, Ring16], m![1 # 8]> = device
        .main
        .begin(reduced.view())
        .fetch::<m![1], m![1 # 8]>()
        .collect::<m![1], m![1 # 8]>()
    ...
    let weight_vrf: VrfTensor<f32, Chip, Cluster, m![Rep16, H / 240], m![H % 240]> = device
        .sub
        ...
        .collect::<m![H / 8 % 30], m![H % 8]>()
        .to_vrf();
    let rms_vrf: VrfTensor<f32, Chip, Cluster, m![Rep16, H / 240], m![1 # 8]> = device
        .sub
        .begin(rms.view())
        .fetch::<m![1], m![1 # 8]>()
        .collect::<m![1], m![1 # 8]>()
        .to_vrf();
    let normalized: DmTensor<f32, Chip, Cluster, m![Rep16, H / 240], m![H % 240]> = device
        .main
        .begin(xpool.view().tile::<m![Slot], 1, m![Slot = 1 # 2, H % 240]>(0))
        .fetch::<m![Slot = 1, H / 16 % 15], m![H % 16]>()
        .fetch_cast::<f32>()
    ...
    let q1_pieces: DmTensor<f8e4m3, Chip, Cluster, m![Rep16, H / 240], m![H % 240]> = device
        .main
        ...
        .commit_trim::<m![H % 8]>()
        .commit();
    let q1_vrf: VrfTensor<f32, Chip, Cluster, m![Rep16, H / 240], m![H % 240]> = device
        .sub
        ...
        .collect::<m![H / 8 % 30], m![H % 8]>()
        .to_vrf();
    let residual: DmTensor<f32, Chip, Cluster, m![Rep16, H / 240], m![H % 240]> = device
        .main
        .begin(normalized.view())
        .fetch::<m![H / 8 % 30], m![H % 8]>()
        .collect::<m![H / 8 % 30], m![H % 8]>()
    ...
    gather_f8(device, &normalized, &mut terms, 0);
    gather_f8(device, &residual, &mut terms, 1);
    terms
}
