...
type Levels = DmTensor<f8e4m3, Chip, OutputClusters, SlidingOutputColumns, m![Lq, Qs % 512]>;
...
pub(crate) type XTrf = TrfTensor<f8e4m3, Chip, OutputClusters, SlidingOutputColumns, m![Lq], m![Qs % 512]>;
pub(crate) type Z = DmTensor<bf16, Chip, OutputClusters, Rows, m![H % 60]>;

pub(crate) type WeightTile = DmTensor<f8e4m3, Chip, OutputClusters, SlidingOutputColumns, m![H % 60, Qs % 512]>;
...
pub(crate) fn contract_tile(device: &mut Device, weight: &WeightTile, x_trf: &XTrf, z: &mut Z) {
    device.main
        ...
        .collect::<m![H % 60, Qs / 32 % 16], m![Qs % 32]>()
        .contract_outer::<m![H % 60, Qs / 64 % 8], m![Qs % 64], _, _, _>(x_trf)
        .contract_packet::<m![1]>()
        .contract_time::<m![H % 60]>()
        .contract_lane::<m![H % 60, Lq], m![1 # 8]>(LaneMode::Sequential)
        .vector_init()
        .vector_intra_slice_tag(TagMode::Zero)
        .vector_narrow_trim::<m![1 # 4]>()
        .vector_intra_slice_reduce::<Lq, m![H % 60], m![1 # 4]>(IntraSliceReduceOpF32::Add)
        .vector_widen_pad::<m![1 # 8]>()
        .vector_inter_slice_reduce::<Rows, m![H % 60]>(InterSliceReduceOpF32::Add)
        .vector_final()
        .cast::<bf16, m![1 # 16]>()
...
pub(crate) fn quantise_x(
    device: &mut Device,
    x: &DmTensor<bf16, Chip, OutputClusters, SlidingOutputColumns, m![Qs % 512]>,
) -> XTrf {
    ...
    device.main
        ...
        .commit_trim::<m![Qs % 8]>()
        .commit_view(levels.view_mut().tile::<m![Lq], 1, m![Lq = 1 #{!} 2, Qs % 512]>(0));
    let q0_vrf: VrfTensor<f32, Chip, OutputClusters, SlidingOutputColumns, m![Lq = 1, Qs % 512]> = device
        .sub
        .begin(levels.view().tile::<m![Lq], 1, m![Lq = 1 # 2, Qs % 512]>(0))
        .fetch::<m![Lq = 1], m![Qs % 512]>()
        .fetch_cast::<f32>()
        .collect::<m![Lq = 1, Qs / 8 % 64], m![Qs % 8]>()
        .to_vrf();
    device.main
        .begin(x.view())
        .fetch::<m![1], m![Qs % 512]>()
        ...
        .cast::<f8e4m3, m![Qs % 8 # 32]>()
        .commit_trim::<m![Qs % 8]>()
        .commit_view(levels.view_mut().tile::<m![Lq], 1, m![Lq = 1 #{!} 2, Qs % 512]>(1));
    device.sub
        .begin(levels.view())
        .fetch::<m![Lq], m![Qs % 512]>()
        .collect::<m![Lq, Qs / 32 % 16], m![Qs % 32]>()
        .to_trf()
}
...
pub(crate) fn project_normalize_add(
    ...
    let x: DmTensor<bf16, Chip, OutputClusters, m![H / 60 % 32, Ns], m![Gs, Ds]> = x.to_dm(&mut device.tdma);
    let x: DmTensor<bf16, Chip, OutputClusters, SlidingOutputColumns, m![Qs % 512]> = unsafe { x.reshape() };
    ...
    let inv_rms: VrfTensor<f32, Chip, Vc, m![1 # 32, Vr], m![1 # 8]> = device
        .main
        .begin(partial.view())
        .fetch::<m![1], m![1 # 8]>()
        .collect::<m![1], m![1 # 8]>()
